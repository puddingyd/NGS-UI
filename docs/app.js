// ------------------------------------------------------------------
// VCF Analysis web tool
// PR 2: read-only from GitHub (private repo via PAT), UI + local edits.
// Save button logs to console — actual GitHub writeback lands in PR 3.
// ------------------------------------------------------------------

const REPO_OWNER = "puddingyd";
const REPO_NAME  = "vcf-analysis-hg38-R";
// Change to "main" once merged. For now we read from the feature branch.
const BRANCH     = "claude/review-vcf-pipeline-iL3BY";
...
async function ghFetchJSON(path) {
  ...
  const url = `${API_BASE}/contents/${path}?ref=${encodeURIComponent(BRANCH)}`;
...
async function loadIndex() {
  const data = await ghFetchJSON("webdata/index.json");
  state.index = Array.isArray(data) ? data : [];
  const opts = await ghFetchJSON("webdata/options.json").catch(() => null);
  state.options = opts && typeof opts === "object" ? opts : { category_options: [] };
}
...
async function loadSample(LIS_ID) {
  const data    = await ghFetchJSON(`webdata/variants/${LIS_ID}.json`);
  if (!data) throw new Error(`找不到 webdata/variants/${LIS_ID}.json`);
  const reports = await ghFetchJSON(`webdata/reports/${LIS_ID}.json`) || {
...
const CANDIDATE_SECTION_DEFS = [
  { el: "cat-candidate",        title: "Candidate",         category: "candidate",        dropdown: "candidate" },
  { el: "cat-diagnostic",       title: "Diagnostic",        category: "diagnostic",       dropdown: "candidate" },
  { el: "cat-pathogenic",       title: "Pathogenic",        category: "pathogenic",       dropdown: "candidate" },
  { el: "cat-clinvar-upgraded", title: "ClinVar upgraded",  category: "clinvar_upgraded", dropdown: "candidate" },
...
document.addEventListener("click", ev => {
  ...
  } else if (t.matches(".btn-export-html")) {
    exportAnalysisHTML();
  } else if (t.matches(".btn-export-clinical")) {
    exportClinicalReport();
  } else if (t.matches(".btn-export-screening")) {
    exportScreeningReport();
  } else if (t.matches("#btn-reset-token")) {
    clearToken();
    showTokenModal();
...
async function saveChanges() {
  ...
  hint("儲存中…");
  try {
    // Yield = 1 iff any variant is marked Causative (status === "1")
    // OR there's a manual Causative variant with a non-empty position.
    ...
    state.reports.updated_at = new Date().toISOString();
    const path = `webdata/reports/${state.currentLIS}.json`;
    await ghPutJSON(path, state.reports, `report: update ${state.currentLIS}`);
    state.dirty = false;
...
document.getElementById("token-save").addEventListener("click", async () => {
  ...
  if (!token) { showTokenModal("請貼上 token"); return; }
  const ok = await verifyToken(token).catch(() => false);
  if (!ok) { showTokenModal("Token 驗證失敗（權限或名稱錯誤？）"); return; }
  setToken(token);
  hideTokenModal();
  await bootAfterAuth();
});
...
(async function boot() {
  ...
  const ok = await verifyToken(token).catch(() => false);
  if (!ok) { clearToken(); showTokenModal("舊 token 無效，請重新輸入"); return; }
  await bootAfterAuth();
})();
