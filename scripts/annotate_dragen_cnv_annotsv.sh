...
# DRAGEN convention: <sample>.{hard-filtered,cnv,sv,cnv_sv}.vcf.gz all
# live in the same directory; sample base is the prefix before
# `.hard-filtered`. We use the dedicated cnv.vcf.gz + sv.vcf.gz
# instead of the legacy cnv_sv.vcf.gz so the two callers' event sets
# don't overlap — no subtraction needed.
DRAGEN_DIR=$(dirname "$DRAGEN_VCF")
BASE=$(basename "$DRAGEN_VCF" .hard-filtered.vcf.gz)
CNV_VCF="$DRAGEN_DIR/$BASE.cnv.vcf.gz"
SV_VCF="$DRAGEN_DIR/$BASE.sv.vcf.gz"
