...
| `download_giab_strata.sh` | 在 dev 機跑一次：下載 GIAB v3.1 GRCh38 stratification 的精選子集（homopolymer / tandem repeat / segdup / low mappability / GC extreme / other difficult）到 `NGS_UI_GIAB_STRAT_DIR` 並產生 `strata_manifest.json`。BED 大、不進 git。用法 `scripts/download_giab_strata.sh [--dir <out>] [--force]`，可用 `GIAB_STRAT_BASE_URL` 覆寫下載來源。 |
