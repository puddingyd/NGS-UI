...
| `annotate_acmg_genebe.py` | GeneBe ACMG 第二意見：選候選點（skip `*` / `GNOMAD_G_AF>0.01` / candidate BED 外）後，用 **`tabix` 對本機 `genebe_hg38.tsv.gz` 離線點查**（依 `#` header 欄名取 `acmg_score`/`acmg_criteria`，非欄號；防呆跳過 pos 非整數的壞行），class 仍用本地 `classify(score)` 五級門檻，寫回 `GENEBE_ACMG_SCORE/_CRITERIA/_CLASS`（只填空格、不碰 pipeline `ACMG_*`）。**取代舊的 GeneBe API（pygenebe via apptainer）**：DB-only、無 API fallback、不需 creds/網路。前置檢查 `tabix` 在 PATH、`.tbi` 存在且**比 `.gz` 新**，否則報錯離開。DB 路徑 `--genebe-db` / `NGS_UI_GENEBE_DB`（預設 `$HOME/NGS_UI/biotools/genebe/genebe_hg38.tsv.gz`）。 |
...
- GeneBe `GENEBE_ACMG_*` 現在來自本機 tabix DB（`annotate_acmg_genebe.py`，取代 API）。DB 是預算好的快取，**miss（如 novel coding indel）不 fallback API**，那些點就沒有第二意見（pipeline `ACMG_CLASS` 仍在）。DB 是 `genebe_hg38.tsv.gz`（bgzip + tabix），踩雷點：① `.tbi` 必須比 `.gz` 新，否則隨機存取讀到「Invalid BGZF header at offset」（stale index，非檔案損毀），要 `tabix -s 1 -b 2 -e 2 -c '#'` 重建；② DB 偶有 `chr.`/pos=`.` 的 placeholder 壞行會讓 `tabix` 建索引失敗，建索引前要 `awk -F'\t' 'NR==1||$2~/^[0-9]+$/'` 濾掉；③ 比對靠欄名（讀 `#` header），slim 7 欄與 full 55 欄 DB 通用。
