...
- **PGx / PharmCAT 卡片**：`/api/samples/{id}/pgx` 回 `pgx` payload，前端 `renderPharmcatBlock` 同時支援 `state.data.pgx` 與 legacy `state.data.pharmcat`。payload 的 `report_view` 由 `docx_export.build_pgx_report_view()` 產生，與健檢 DOCX 共用固定 21-gene scope、actionable/source priority、摘要與 genotype/phenotype fallback。報告區是健檢輸出預覽；分析區先顯示六類完整用藥概覽與完整建議，再以固定 gene order 顯示各 gene 相關藥物、相同格式建議及 allele/variant evidence；需注意表型標紅。normal/uncertain/unknown/no-result 歸不需調整，相關藥物仍列出。PharmCAT object message 不渲染成 `[object Object]`；variant 的 allele 清單標為可參與定義的 star alleles，長清單預設收合，不得解讀為病人同時具有全部 allele。MT-RNR1 JSON raw `Unknown` allele placeholder 隱藏，但允許 `pgx.tsv` 補 diplotype/risk；除 MT-RNR1 外不得用 TSV 補其他 gene。左側 sidebar 入口文字是 `Pharmacogenomics`。
