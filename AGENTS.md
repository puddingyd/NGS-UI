...
- **三級分析 modal / job log**：in-house 與 DRAGEN VCF 各自使用 typeahead，支援同來源批次；worker 保存 `source_sample_id`，用 v3.x samplesheet 執行 `nextflow ... --out_dir /home/datalake_Intermediate/pipeline/tertiary_output -resume`。Nextflow 產出 03-07 後 UI 直接讀取，不複製 SNV/CNV/SV/STR/PGx。post-processing 在 `08_postprocessing` 建暫存 working SNV，跑 GeneBe/extra-VEP/GIAB/院內 AF/MANE 後建立 sparse overlay，再以 03 raw 建 review/index，最後刪 working 並寫 `layout.json`；若 06 缺 CNV/SV，本機 AnnotSV fallback 才寫入 08。新樣本 marker 完成前不列入「載入新個案」。進度為 Nextflow 3-82%、prepare/post-processing/overlay/review/index 82-99%；清單合併新 root、legacy pipeline root 與 job state。完整刪除才會刪 pipeline/UI tree；個案清單刪除只 unregister。
