...
snv_indel.review.tsv          主畫面用衍生檔（自動重建；AF<0.05 / AF缺值 / ClinVar P/LP / reviewer 已標記）
snv_indel.review.tsv.source.json  衍生檔 manifest（raw mtime/size + rescue IDs + AF 門檻）
...
- **個案清單管理**：modal 內有獨立的 `WES` / `WGS` 圓形 chips 與全文搜尋框，會搜尋 ID、姓名、MRN、variant、疾病、comment 與日期等所有表格內容。表格摘要 status `1` 的 causative SNV/CNV/SV、實際勾選的 SNV OMIM diseases 與 CNV/SV reviewer `Disease`、status `2` 的 other SNV/CNV/SV、主畫面 comment、簽收時間與載入時間；多個 variant / disease 逐行顯示，長 HGVS 可在任意字元換行。摘要依 metadata / raw TSV / OMIM signature 使用 bounded LRU，避免每次開啟都重掃完整 TSV。
...
- **載入效能**：`snv_review.ensure_review_tsv()` 自動維護 `snv_indel.review.tsv`，主畫面只預載 `GNOMAD_G_AF < 0.05`、AF 缺值、ClinVar P/LP rescue 與 reviewer 已標記點。`run_stopgaps.sh` 在三級分析結尾先執行 `build_snv_review_tsv.py`；載入時 lazy rebuild 僅作為舊樣本 fallback。`sample_loader` 的 process-local bounded LRU 依 TSV / analysis / pheno / Exomiser / LIRICAL / OMIM signature 自動失效；刪除個案只 invalidate 該 sample 的 SNV 與個案摘要 cache，不同步重掃全部清單。大型 JSON response 用 gzip。OMIM enrichment 每批只檢查一次 xlsx mtime。FastAPI startup 用 daemon thread 背景預熱 HPO、phenotype gene map、OMIM 與 mito ClinVar，避免大型 `phenotype_to_genes.txt` 擋住 HTTP port；完整 SNV TSV gene-search cache 由單一 daemon queue 預熱，restart 不等待尚未完成的預熱工作。
- **三級分析 modal / job log**：in-house 與 DRAGEN VCF 各自使用單一 typeahead 輸入格；點入輸入格即展開全部 VCF，輸入 sample / run / path 後即時縮小候選清單。Sample ID 可修改作為 UI 輸出資料夾與檔名前綴；選好後可按「加入批次」，同一批只能包含同一種來源（全 in-house 或全 DRAGEN），按「開始分析」後以多列 v3.1 sample sheet 一次送出。worker 另外保存原始 `source_sample_id`，v3.1 預設為每個 job 產生 `samplesheet.csv`（`sample_id,pipeline_type,input_dir,seq_type,hpo`），執行 `nextflow ... --samplesheet ... --pipeline_type nckuh|dragen --out_dir /home/pipeline/tertiary_output -resume`；若 `NGS_UI_TERTIARY_ENV_SCRIPT`（預設 `/home/pipeline/pipeline_code/NGS2ndAnalysis_env.sh`）存在會先 source，不存在則沿用目前環境直接執行 `nextflow`。v3.1 pipeline 以 source sample ID 輸出 `/home/pipeline/tertiary_output/{source_sample_id}/03_acmg/{source_sample_id}.snv_indel.acmg.tsv`；NGS-UI 逐筆複製到 `NGS_UI_HOME/tertiary_output/{Sample ID}/snv_indel.annotated.tsv` 並寫 `pipeline_source.json`。只有設定 `NGS_UI_TERTIARY_LEGACY_STAGING=1` 時才回到舊 `stage_dragen_for_tertiary.sh` 單樣本 staging 流程。VCF 建立時間來自檔案 `mtime` Unix timestamp，前端固定以 `Asia/Taipei`（UTC+8）顯示。工具列內的 Extra VEP 只顯示 checkbox，與不換行的 `↻ 更新索引`、`三級分析清單` 固定同高；tooltip 只描述補 SpliceAI 註解，pipeline 行為不變。Extra VEP 和 GeneBe 一樣只送 `GNOMAD_G_AF ≤ 0.01` 或 AF 缺值候選點執行，再把 annotation merge 回完整 TSV。執行進度顯示細分進度條：queued/mito 1%、detect/samplesheet 2%、Nextflow 3-55%（PREPARE_VCF / PREPARE_VCF_DRAGEN 3%、VEP 13%、PANGOLIN 26%、PARSE CSQ 49%、ACMG 52%）、copy 55%、stop-gaps 55-100%（ClinVar fallback 55%、filter 65%、GeneBe 66%、extra VEP 70%、AnnotSV 78%、review TSV 98%、done 100%）。Nextflow stdout 與 stop-gaps marker 會即時更新 state step；黑底 log 預設收合，按右側三角形展開。worker 自己寫的 log 行與 stop-gaps 子步驟帶 ISO timestamp，子程序結束時記錄 elapsed seconds；`state.json` 另存 `step_started_at` + `step_history`。modal 內的「三級分析清單」合併 `PIPELINE_OUT_ROOT` 與 NGS-UI job state，因此失敗且沒有 output 目錄的 sample 仍會顯示 log；刪除時會一併刪除 `/home/pipeline/tertiary_output/{sample}/`、`NGS_UI_HOME/tertiary_output/{sample}/`；batch job log 會保留給同批其他 sample 查詢。新 job 狀態寫到 `data/jobs/tertiary/`；舊版 `data/jobs/dragen/` 僅作 read-only fallback。`run_stopgaps.sh` 不再建立 `.raw` snapshot；GeneBe 暫存 VCF 帶 primary contig header，AnnotSV 成功執行時只寫摘要 log，失敗才附尾端診斷。
