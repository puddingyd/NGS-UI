...
- **三級分析 output 來源分流 / PGx 補跑**：新邏輯優先以 UI sample ID（如 `{source}-dragen` / `{source}-inhouse`）管理 `/home/pipeline/tertiary_output/`，避免 DRAGEN 與 in-house 同名 sample 互相覆蓋；舊的 source-ID-only output 只作 legacy fallback，並可用 `PYTHONPATH=backend python scripts/repair_pipeline_output_suffixes.py --apply` 依 `pipeline_source.json` 複製成 suffixed 目錄。Nextflow reuse 不再只看 `03_acmg/*.snv_indel.acmg.tsv`，而是檢查 SNV、Mito、CNV、SV；PGx checkbox 有勾時也必須偵測到 PGx/PharmCAT 輸出，少任一項就執行 Nextflow `-resume` 補齊。
