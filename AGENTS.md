...
- **個案清單管理**：modal 內有獨立的 `WES` / `WGS` 圓形 chips 與全文搜尋框，會搜尋 ID、姓名、MRN、variant、疾病、comment 與日期等所有表格內容。表格摘要 status `1` 的 causative SNV、實際勾選的 OMIM diseases、status `2` 的 other SNV、主畫面 comment、簽收時間與載入時間；摘要依 metadata / raw TSV / OMIM signature 使用 bounded LRU，避免每次開啟都重掃完整 TSV。
