...
- **三級分析 modal / job log**：in-house 與 DRAGEN VCF 各自使用單一 typeahead 輸入格；點入輸入格即展開全部 VCF，輸入 sample / run / path 後即時縮小候選清單。Sample ID 可修改作為輸出資料夾與檔名前綴；若 in-house 輸出 ID 與 VCF 原始 ID 不同，worker 會先用 `stage_dragen_for_tertiary.sh --skip-norm --skip-bed --skip-gnomad --keep-chrm` 建立只改 `_DV` / `_HC` column prefix 的 staging VCF，再交給正式 Nextflow scripts。VCF 建立時間來自檔案 `mtime` Unix timestamp，前端固定以 `Asia/Taipei`（UTC+8）顯示。工具列內的 Extra VEP 只顯示 checkbox，與不換行的 `↻ 更新索引`、`三級分析清單` 固定同高；tooltip 只描述補 SpliceAI 註解，pipeline 行為不變。Extra VEP 和 GeneBe 一樣只送 `GNOMAD_G_AF ≤ 0.01` 或 AF 缺值候選點執行，再把 annotation merge 回完整 TSV。執行進度顯示細分進度條：queued/mito 1%、detect/stage 2%、Nextflow 3-55%（依序 ADD_CALLERS 3%、FILTER 8%、VEP 13%、PANGOLIN 26%、PARSE CSQ 49%、ACMG 52%）、copy 55%、stop-gaps 55-100%（ClinVar 55%、filter 65%、GeneBe 66%、extra VEP 70%、AnnotSV 78%、review TSV 98%、done 100%）。Nextflow stdout 與 stop-gaps marker 會即時更新 state step；黑底 log 預設收合，按右側三角形展開。worker 自己寫的 log 行與 stop-gaps 子步驟帶 ISO timestamp，子程序結束時記錄 elapsed seconds；`state.json` 另存 `step_started_at` + `step_history`。modal 內的「三級分析清單」合併 `PIPELINE_OUT_ROOT` 與 NGS-UI job state，因此失敗且沒有 output 目錄的 sample 仍會顯示 log；刪除時會一併刪除 `/home/pipeline/tertiary_output/{sample}/`、`NGS_UI_HOME/tertiary_output/{sample}/` 與該 sample 的 NGS-UI job log 目錄，執行中的 sample 拒絕刪除。新 job 狀態寫到 `data/jobs/tertiary/`；舊版 `data/jobs/dragen/` 僅作 read-only fallback。`run_stopgaps.sh` 不再建立 `.raw` snapshot；GeneBe 暫存 VCF 帶 primary contig header，AnnotSV 成功執行時只寫摘要 log，失敗才附尾端診斷。
...
| `filter_snv_tsv.py` | GUI TSV 的結構性清理：只移除 `REF/ALT=*` 與非 primary contig；AF、VAF、IMPACT 留在 TSV，由 UI 顯示 filter 控制。DRAGEN staging 仍會先移除 `AF > 0.01` 的 common variants |
