...
| **PGx** | `pgx_tsv.py` + `docx_export.build_pgx_report_view` | gene / drug | PGx / PharmCAT 區塊 | `pharmcat.report.json` 是主資料源：adapter 保留精簡 gene details、variants、uncalled/messages 與 CPIC/FDA annotations，再由健檢 DOCX 共用 presenter 產生固定 21 個 CPIC Level A genes 的概覽、摘要、基因型/表型與完整建議；`pgx.tsv` 只允許在 JSON 的 MT-RNR1 為 Unknown/No Result 時補值。 | 報告區依藥物 A-Z；分析區完整建議 + 固定 gene order |
