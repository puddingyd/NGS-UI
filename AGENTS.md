...
- **OMIM disease list**（SNV 卡片，`renderDiseaseList`）：`<details>` 列出 `Disease1..5`（跳過 NA），summary 有報告勾選 checkbox（→ `state.reports.edits[id].report_diseases`），展開的黃底框（`.disease-detail`）底部有「▴ 收合」鈕。`omim_store.compact_disease_label()` 與前端 `compactDiseaseLabel()` 會把 curator 長篇說明截到第一個可辨識的遺傳模式括號（如 `(AD)`、`(AR)`）結束，保留原始 `OMIM.xlsx` / `OMIM_disease` 不動。
