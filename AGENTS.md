...
- **性別 / ploidy 核對**：DRAGEN 以原始 VCF basename/source ID 精確找同層 `*.ploidy.vcf.gz`；NCKUH 從 `04_snv_indel/{source}.ensemble.fixed.vcf.gz` 的 sibling `03_alignment_qc/{source}.ploidy.vcf.gz` 找。VCF 複製成 `08_postprocessing/{LIS_ID}.ploidy.vcf.gz`；同層或 `00_prepare/{source}.ploidy_qc.txt` 也複製成 prefixed QC 留待未來，現階段 UI/報告不得讀 QC TXT。payload 解析 header 與全部 contig FILTER/END/DC/NDC/RATIO。色彩優先：非 XX/XY 或 chr1-22/X/Y 任一非 PASS＝紫色；否則病歷性別空白/U/不符＝紅色；M↔XY、F↔XX＝綠色。`ploidy VCF: {call}` 不論符合與否都顯示，點擊 modal 顯示全部 chromosome；SUSPECT 只代表可能 dosage anomaly、不是確診。
