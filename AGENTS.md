...
- ACMG SF / 血脂相關基因 / 腫瘤相關基因 / 中風相關基因 / Carrier / Proactive / PGx / PharmCAT —— 收在「Secondary findings」折疊群組（純文字標題 + 三角形鈕，無卡片框）。SNV secondary panels 依序為 `ACMG_SF_v3.3`、`lipid_fh`（`LDLR/APOB/PCSK9`）、`WES-I__腫瘤醫學__遺傳癌症`、`WGS__神經科__Stroke`、`carrier_mackenzie_1300+`、`proactive`；後端從 `snv_indel.review.tsv` 產生候選點，再套 panel gene、ClinVar P/LP 或卡片最終 ACMG P/LP（GeneBe 優先、再 fallback pipeline ACMG）、VAF ≥ 0.2、zygosity 非 ref，不掃完整 raw TSV 也不報 BED 外位點。這些 SNV secondary 區透過 `/api/samples/{id}/secondary-snv` staged loading 背景載入，核心 `/samples/{id}` 不等待 secondary finding 計算；PGx 透過 `/api/samples/{id}/pgx` staged loading 背景載入。secondary analysis 區與 report 區預設展開；analysis 區保留全部候選點位，report 區只顯示 ✓ 點位。ClinVar P/LP 預設 ✓，非 ClinVar 但 ACMG P/LP 預設不 ✓；使用者手動勾選寫入 `reports.secondary_findings.{section}.selected`，手動取消寫入 `.dismissed`，所以之後不會再被 ClinVar 預設規則勾回。舊 `reports.panels[id][section] == "V"` / `"0"` 只作讀取相容。
...
- Secondary findings 的 SNV secondary 卡片只顯示一顆同樣圓形樣式的紫色 **`✓`**；有 ✓ 進 Secondary findings 報告區，取消或未勾者仍留在分析區。這個 ✓ 與 `1/2/C/0` 完全獨立，同一 variant 即使已在 causative / other / candidate 仍可列入 secondary report 區；同一點位在分析區與報告區手動改 ACMG / comment 時要同步。
- 「匯出健檢報告」在診斷報告 DOCX 與輸出 PDF 按鈕旁，會先跳出選擇畫面；預設勾選 ACMG SF 與藥物基因體學，血脂/腫瘤/中風/Carrier/Proactive 預設不勾。DOCX 保留原章節骨架與既有固定寬度 `====` ASCII 變異表格；頁首保留院名與「基因醫學部基因檢測分析研究報告」副標題。檢驗套組依 UI 勾選顯示 ACMG疾病風險基因 / 藥物基因體學，UI 彈窗選單顯示「ACMG 疾病風險基因」。ACMG SF 固定警語列次發現基因清單第 3.3 版（2025）、ClinVar 版本及已收錄 P/LP 回報範圍；主文分成「第一類：與疾病風險相關之致病性或疑似致病性變異位點」與「第二類：符合帶因者狀態之致病性或疑似致病性變異位點」。AD、X-linked、純 AR 同基因兩個以上變異、純 AR 單一 homozygous 變異列第一類；純 AR 單一 heterozygous 變異列第二類；未知或混合遺傳模式不視為帶因者，保守列第一類。第一類陰性句固定為「於本次檢測之基因中，未檢出疾病資料庫中已收錄之致病性或疑似致病性變異。」；第二類陰性句固定為「於本次檢測之基因中，未檢出疾病資料庫中已收錄且符合帶因者狀態之致病性或疑似致病性變異。」原本六組分類標題與完整基因移到 §五基因清單。同基因多位點合併在同一表格，結構欄沿用 transcript 的 EXON/INTRON rank，EXON 為 `. / - / NA` 時 fallback INTRON，並依所有列的 ACMG 分級產生一次綜合三點說明；五級分類中文統一為良性、疑似良性、不確定意義、疑似致病性、致病性。AR 單一 heterozygous 變異明確於第 1 點寫「本次僅檢出一個符合報告條件之變異，檢測結果符合帶因者狀態」；AR 多變異一律相位未知，第 2 點寫明「此為...之變異位點」後以句號連接「由於兩變異之相位尚未確認」，並建議進行家族成員檢測，以釐清兩變異是否位於不同等位基因，方能判斷是否符合體染色體隱性疾病之雙等位基因致病型態；X-linked 只依 XX/XY 分流，女性單一變異採可能受 X 染色體失活型態及疾病表現範圍影響的保守文字；建議句使用「遺傳諮詢或門診相關專科」。完整檢測限制在 ACMG SF 全段末尾只輸出一次，所有變異位點參考資料依第一類、第二類主文順序集中在整份報告末端附錄。健檢 PGx 建議文字與 CPIC/FDA annotations 主要使用 `pharmcat.report.json`；主文順序為「用藥建議概覽」、「藥物建議摘要」、「基因型與表現型」，完整用藥建議移到末端附錄。固定 21 個 CPIC Level A genes 的基因型與表現型以 JSON gene details 為主，但 MT-RNR1 在 JSON 為 Unknown/No Result 時可由 `pgx.tsv` 補基因型/表型；phenotype 欄優先使用 JSON phenotype，`n/a` 或空值時改用 allele function，兩者皆無時顯示 `No phenotype assigned`。IFNL3 排除於 PGx 主文、drug-centric 完整用藥建議附錄及 §五基因清單。「用藥建議概覽」保留「其餘未列之藥物，未發現符合本報告回報規則之明確處方調整建議。」藥物建議摘要以固定寬度 `====` 表格列出「藥物 / 基因 / 建議處置 / 建議依據及等級」，一個藥物一列並依藥物英文名稱 A-Z 排序，且只列 CPIC Strong/Moderate 與 FDA therapeutic-management，FDA-label-only 藥物只留在完整建議附錄。完整用藥建議只保留 actionable CPIC Strong/Moderate、FDA Label 與 FDA Section 1，排除 normal、uncertain susceptibility、standard dosing、no-action；表格總寬與基因型表格對齊，同一藥物只列一次，CPIC/FDA 建議合併於同一列，同一 drug/source/action 有 CPIC Strong/Moderate 時保留 Strong；CPIC 與 FDA 來源皆放在各段 recommendation 句尾括號，FDA 內容先列 therapeutic management、再列 label，並移除外層引號與 PharmCAT/FDA label 原文省略符號。附錄另列 CPIC/ClinPGx、FDA PGx 關聯表、FDA 生物標記表與 Drugs@FDA 官方可點擊網址及用藥警語。§五固定列目前 21 個至少有一組 CPIC Level A gene–drug pair 的基因，標題與參考依據不列 FDA；暫不依 validation scope 刪減。
- 健檢 ACMG SF 的參考資料順序必須跟上方點位顯示順序一致；同一基因多位點在上方表格合併時，參考資料也連續列出。
- 健檢報告目前的版面細節：ACMG 共用警語置於第一、第二類標題之前；第一類標題與結果之間不留空行，第一類結果與第二類標題之間保留一行。變異表格的 ACMG/AMP 欄維持 PharmCAT/來源資料的英文分級，中文分級只用於表後說明與附錄參考資料。PGx 摘要與完整建議只列實際驅動處置的基因，正常、低風險或不確定表型不會因同一藥物的另一基因需調整而併列；藥物欄採英文單字不拆行。CPIC 與 FDA 官方資訊置於 PGx 主文末端、§四之前，CPIC 最新臨床指引列第一項，紙本可直接閱讀完整 URL；報告末端附錄不編號，依序為「變異位點參考資料」及「完整用藥建議」，兩者之間保留一行。WGS 健檢報告的平均深度為 27X，限制段落包含 CNV 未涵蓋範圍與 CYP2D6 藥物基因體學專一納入 CNV 的例外，但不再寫「ACMG SF 或」；診斷報告仍使用 30X。所有 ASCII 表格不設定 keep-with-next，避免 Word 顯示段落左側黑色方塊。
...
- **Exomiser/LIRICAL rerun worker**（`workers/exomiser_lirical.py`）：渲染 repo 內 git-tracked `phenotype_reference/exomiser_input.yml`/`lirical_input.yaml` 模板 → java -jar 跑 → 結果寫 `analyses/{ver}/exomiser_results.tsv`/`lirical_results.tsv`；render 階段失敗會把 traceback 寫入該 version 的 `analysis_files/rerun.log`。**不算 pheno_score**（那是 `write_version` 的事）。RQ/Redis job queue。
- **secondary panel 檔**：`phenotype_data/custom_panels/ACMG_SF_v3.3.txt`、`lipid_fh.txt`、`carrier_mackenzie_1300+.txt`、`proactive.txt`，以及 `phenotype_data/gene_panels/WES-I__腫瘤醫學__遺傳癌症.txt`、`WGS__神經科__Stroke.txt`；`docs/` 另保留舊版分析網頁的 `app.js/index.html/style.css`（參考用）。
...
- SNV/Indel 卡片的 Score、ClinVar、ACMG 與每個 in-silico tool 旁都有 `ⓘ` hover/focus 註解。ACMG 註解需動態顯示 `source: manual/GeneBe/in-house` 並列其他來源分類；predictor 註解需顯示本位點 evidence、threshold 及可點擊 Reference URL/PMID。只有有正式 calibration 的工具可標 PP3/BP4 strength；P-KNN 以 `PKNN_EVIDENCE` 為準，未校準工具只能標 model cutoff/contextual evidence。SIFT 多值必須取 minimum（越低越 deleterious）。
