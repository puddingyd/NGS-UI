...
snv_indel.review.tsv          主畫面用衍生檔（自動重建；SNV/Indel 會排除 chrM/MT，WES 先直接濾 DP<20，再取 candidate BED 內 AF<0.01 / AF缺值 / ClinVar P/LP；reviewer 已標記 SNV 另由 index 補入且同樣套 WES DP≥20）
...
str.tsv                       STR（三級 pipeline `05_str/{source_sample_id}.str.tsv` 複製而來；`no_threshold` 和 normal 同 tab 顯示）
pgx.tsv pharmcat.report.json  PGx / PharmCAT（三級 pipeline `07_pgx/` 複製而來；前端以 `pgx.tsv` 聚合，JSON 只取版本/時間/基因細節）
...
| **PGx** | `pgx_tsv.py` | gene | PGx / PharmCAT 區塊 | 讀 `pgx.tsv` 聚合 gene → drug recommendations；`pharmcat.report.json` 只抽 PharmCAT version/data version/timestamp、非 reference variant details、uncalled/messages，不把完整 JSON 放進核心 payload。不在 `pgx.tsv` 但在 PharmCAT JSON 的 gene 會進 Additional PharmCAT genes 折疊區。actionable 規則：HLA positive、MT-RNR1 HIGH、非 normal/unknown 的 metabolizer/function 且有 Strong/Moderate 非 no-action recommendation 等；HLA negative、MT-RNR1 LOW、Uncertain Susceptibility 放 Routine。 | Actionable 優先，再依 evidence 強度與 gene |
...
| `build_snv_review_tsv.py` | 從完整 `snv_indel.annotated.tsv` 預建主畫面用 `snv_indel.review.tsv`；支援 `--test-type WES|WGS`，排除 chrM/MT，WES 會直接濾 DP<20；`run_stopgaps.sh` 結尾自動執行並傳入 seq type |
