...
3. **看變異卡片** — 個案載入後先顯示 SNV/Indel（分段載入），CNV/SV、Mitochondria、STR 與 PGx 在背景載完後補上：
   ...
   - 「匯出健檢報告」會先跳出選擇畫面，預設勾 ACMG SF 與藥物基因體學，血脂/腫瘤/中風/Carrier/Proactive 預設不勾。DOCX 保留原診斷報告名稱及章節骨架、固定寬度 `====` ASCII 表格、表格下方列點、檢測方法、注意事項與基因清單；檢驗套組會依 UI 勾選自動組合「可採取醫療處置之疾病風險基因」與「藥物基因體學」。ACMG SF 固定警語說明 v3.3、ClinVar 版本及 P/LP 回報範圍，表後依 AD、AR、X-linked 與性染色體組成自動產生三點中文說明，完整檢測限制只在 ACMG SF 全段末尾輸出一次。PGx 摘要將英文建議自動歸納為中文動作句；下方分列 TSV 產生的 CPIC 完整建議與 PharmCAT JSON 產生的 FDA therapeutic-management 資訊。
   ...
   - 健檢 DOCX：ACMG SF 維持既有 ASCII 變異表格，表後依 AD、AR、X-linked、zygosity、同基因回報變異數及性染色體組成自動產生結果意義與建議；性染色體組成優先讀取已複製的 `ploidy.vcf.gz` 之 `estimatedSexKaryotype`，其次使用 EMR 性別，無資料時採中性文字。X-linked 女性採可能無症狀或因疾病特性及 X 染色體失活而出現臨床表現的保守文字。PGx 先輸出 CPIC Strong/Moderate 與 FDA therapeutic-management 摘要，並將摘要 recommendation 歸納成中文動作句；CPIC 完整建議由 TSV 產生，FDA therapeutic-management 則由 PharmCAT JSON 獨立產生完整基因型／表型／用藥資訊，因此 `VKORC1` 等 JSON-only gene 不需出現在 TSV。末尾 PGx 基因清單自動合併 TSV pipeline call 與 PharmCAT JSON FDA 結果基因，參考依據同列 CPIC 與 FDA。
