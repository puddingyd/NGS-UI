...
4. **標記與判讀** — 在每個變異上標 causative / candidate / other，編輯 ACMG/分類、寫 comment；SNV/Indel ACMG 優先序為 reviewer override → GeneBe → pipeline `ACMG_CLASS`。變更會自動存到 `{LIS}/08_postprocessing/{LIS}.sample_metadata.json`（舊個案仍可寫 legacy 名稱）；開啟個案清單或匯出 DOCX 前會先 flush 尚未完成的自動儲存。
...
三級分析 modal 的 in-house 與 DRAGEN VCF 各自使用單一 typeahead。worker 為 v3.x pipeline 產生 sample sheet，執行 `nextflow ... --out_dir /home/datalake_Intermediate/pipeline/tertiary_output -resume`。pipeline 完成後，UI 直接讀 03-07，不再複製 SNV/CNV/SV/STR/PGx；Mito 只有本機 MITOMAP 確實補值時才在 08 留小型衍生檔，DRAGEN/NCKUH ploidy 與 reviewer state 也放在 08。SNV post-processing 會暫時複製 raw 成隱藏 working TSV，完成 GeneBe/extra-VEP/GIAB/院內 AF/MANE 後建 prefixed overlay，並在 `finally` 刪除 working TSV；因此不會永久留第二份數 GB TSV。接著以 03 raw 建 prefixed review/index，非破壞性補齊其餘 v3 prefixed state，最後才原子寫入 `{LIS_ID}.layout.json` 讓 UI 看見個案。若 06 已有 CNV/SV 就直接讀，缺檔才把本機 AnnotSV fallback 寫入 08。三級分析清單同時顯示新 root、legacy `/home/pipeline/tertiary_output` 與 job state；完整刪除會清掉對應 pipeline/UI tree 與 job log，執行中的 sample 不可刪。
