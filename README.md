...
3. **看變異卡片** — 個案載入後先顯示 SNV/Indel（分段載入），CNV/SV 與 Mitochondria 在背景載完後補上：
   ...
   - GeneBe ACMG 第二意見（`GENEBE_ACMG_*` 欄）改由本機 GeneBe 資料庫離線提供，取代原本的 GeneBe API（不需帳號/網路）：`run_stopgaps.sh` 對整張 TSV 的變異查 `NGS_UI_GENEBE_DB`（預設 `biotools/genebe/genebe_hg38.tsv.gz`）的 `acmg_score`/`acmg_criteria`。查詢方式是串流掃整顆 DB 一次（不依賴 tabix `.tbi`，stale/缺索引不會影響）。DB 為預算好的快取，查不到的點（如 novel coding indel）就沒有 GeneBe 第二意見，pipeline 自己的 ACMG 仍會顯示。DB 部署/更新用 `scripts/deploy_genebe_db.sh`（濾壞行 → bgzip → tabix → 原子換檔），建置端要求見 `docs/genebe_db_requirements.md`。
