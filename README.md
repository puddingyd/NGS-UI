...
3. **看變異卡片** — 個案載入後先顯示 SNV/Indel（分段載入），CNV/SV、Mitochondria、STR 與 PGx 在背景載完後補上：
   ...
   - 「匯出健檢報告」會先跳出選擇畫面，預設勾 ACMG SF 與藥物基因體學，血脂/腫瘤/中風/Carrier/Proactive 預設不勾，彈窗選單顯示「ACMG 疾病風險基因」。DOCX 保留原章節骨架、固定寬度 `====` ASCII 表格、表格下方列點、檢測方法、注意事項與基因清單，報告標題為「基因醫學部基因檢測檢驗分析研究報告」；檢驗套組會依 UI 勾選自動組合「ACMG疾病風險基因」與「藥物基因體學」。ACMG SF 固定警語使用次發現基因清單第 3.3 版（2025 年發表）、ClinVar 版本及 P/LP 回報範圍；同基因多位點合併在同一表格並共用一組綜合說明，EXON 為 `.` 等缺值時會改讀 INTRON，ACMG 參考資料移到 ACMG SF 區塊尾端並置於檢測限制警語之前，完整檢測限制只在 ACMG SF 全段末尾輸出一次。健檢 PGx 內容完全取自 `pharmcat.report.json`：重點摘要以固定寬度 `====` ASCII 表格列出「藥物 / 基因 / 建議處置 / 建議依據及等級」，一個藥物一列並依藥物英文名稱 A-Z 排序，且只列 CPIC Strong/Moderate 與 FDA therapeutic-management，FDA-label-only 藥物只留在完整建議；接著列固定 21 個 CPIC Level A genes 的基因型與表現型，phenotype 欄優先使用 JSON phenotype，`n/a` 或空值時改用 allele function，兩者皆無時顯示 `No phenotype assigned`；MT-RNR1 在 JSON 無有效結果時可由 `pgx.tsv` 補基因型/表型，再依 one-sentence 建議處置彙整藥物分類。用藥建議分類後保留「其餘未列之藥物，未發現符合本報告回報規則之明確處方調整建議。」完整用藥建議改為 drug-centric 表格，只列 actionable 藥物，表格總寬與基因型表格對齊，並將同一藥物的 CPIC、FDA Label 與 FDA PGx Association 建議合併在同一列；同一 drug/source/action 有 CPIC Strong/Moderate 時保留 Strong，CPIC/FDA 來源皆改放在各段 recommendation 句尾括號，FDA therapeutic-management 內容先於 FDA label 內容，並清理 PharmCAT/FDA label 原文的引號與省略符號。報告基因範圍固定為目前 21 個至少具有一組 CPIC Level A gene–drug pair 的基因，不含 CYP3A4 與 IFNL3。
   ...
   - 健檢 DOCX：ACMG SF 維持既有 ASCII 變異表格，表後依 AD、AR、X-linked、zygosity、同基因回報變異數及性染色體組成自動產生結果意義與建議；性染色體組成優先讀取已複製的 `ploidy.vcf.gz` 之 `estimatedSexKaryotype`，其次使用 EMR 性別，無資料時採中性文字。AR 單一 heterozygous 變異會寫明「本次僅檢出一個符合報告條件之變異，檢測結果符合帶因者狀態」；AR 多變異會寫明兩變異相位尚未確認，建議進行家族成員檢測以釐清是否位於不同等位基因，方能判斷是否符合體染色體隱性疾病之雙等位基因致病型態。建議句使用「遺傳諮詢或門診相關專科」。X-linked 女性單一變異採可能受 X 染色體失活型態、家族史及疾病表現範圍影響的保守文字。PGx 報告不再以 `pgx.tsv` 產生建議文字，基因型、表型、重點摘要、用藥建議分類與完整 CPIC/FDA 建議主要由 PharmCAT JSON 產生；MT-RNR1 基因型/表型在 JSON 無有效結果時可從 TSV 補值。重點摘要與完整用藥建議皆以藥物 A-Z 呈現，重點摘要不列 FDA-label-only 項目，完整建議同一藥物只列一次且 CPIC/FDA 來源放在建議句尾括號。末尾固定列 21 個 CPIC Level A genes，IFNL3 不會出現在四個 PGx 報告區段或基因清單，標題與參考依據不再列 FDA。
   - 健檢 ACMG SF 的參考資料順序跟上方點位顯示順序一致；同一基因多位點在表格合併時，參考資料也連續列出。
