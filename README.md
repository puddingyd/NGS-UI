...
└── {LIS_ID}/
    ...
    └── 08_postprocessing/
        ...
        ├── {LIS_ID}.litvar2_on_demand.sqlite ← gene search / 手動單點版本化快取
...
最新版 ClinVar UI 比對庫每週三台北時間 04:00 更新（DOCX 仍固定使用 20260720）；安裝器也會在首次安裝時背景啟動一次建庫：

```bash
scripts/install_clinvar_update_timer.sh
systemctl list-timers ngs-ui-clinvar-update.timer
```

三級分析的 DRAGEN / NCKUH VCF 索引每天於台北時間 02:00 自動重掃；UI 仍保留人工更新按鈕，索引不存在或排程失敗時也保留首次建立與 stale fallback：

```bash
scripts/install_tertiary_index_timer.sh
...
| `NGS_UI_AUTOMAP_SIF` | `/home/datalake_Intermediate/pipeline/nextflow_containers/automap_1.3.sif` | in-house 缺 AutoMap 且勾 Research-only 時，三級補跑 AutoMap 使用的 Apptainer image；未設定且主路徑不存在時嘗試舊 `/home/pipeline/nextflow_containers/automap_1.3.sif` |
...
| `NGS_UI_CUSTOM_GENE_PANELS_DIR` | `REPO_ROOT/phenotype_data/custom_panels` | git-tracked custom panel gene lists 與 `panel_metadata.tsv` 名稱／來源／輸出名稱對照表 |
...
| `NGS_UI_GPN_MSA_DB` | `$NGS_UI_HOME/biotools/gpn_msa/scores.tsv.bgz` | 固定 GRCh38 GPN-MSA pre-computed score；所有 case 都嘗試以同路徑 `.tbi` + `tabix` join 最終 review TSV，缺資源或查詢失敗只 warning／記狀態，不阻擋三級 |
...
2. **載入新個案** — 點「載入新個案」：
   - 按「EMR 同步」也會依目前填寫的病歷號，重新帶入「輸入臨床表徵」頁面已儲存的 HPO／gene panel，並顯示 Clinical presentation 唯讀預覽。已儲存的 HPO/panel（含明確清空的內容）優先於 EMR；查無資料才用 EMR HPO。EMR 連線失敗不影響已儲存內容的載入，Clinical presentation 在登錄時仍取該病歷號最新內容。
   ...
   - 主畫面的 Genetic counseling、Clinical presentation 與 Comment 長文字欄位會在載入既有內容、展開卡片及輸入時依 `scrollHeight` 自動增高，完整容納內容而不顯示 textarea 內部垂直捲軸；Comment 右側 Tag picker 維持並排。
   ...
   - 登錄新個案的未登錄個案欄位可直接輸入 LIS ID、source sample、姓名或 MRN 搜尋，也可從下拉清單選擇；清單由伺服器保存共用精簡索引，登入後預先抓取，開啟視窗先顯示已有清單再背景更新；選取檢體後才讀該個案的 HPO／panel 與 VCF 大小，載入期間會提示。伺服器啟動時預建索引，超過 5 分鐘於背景重新掃描；登錄、取消登錄與三級分析完成／刪除即更新相關項目。病歷號連結更新直接合併新對照，不重掃 pipeline。可按「更新清單」強制掃描，背景更新失敗時保留上次清單。登錄完成會同步切換主畫面與上方個案搜尋框。登錄時不再同步掃完整 TSV 產生 `vcf_from_tsv.vcf.gz`；至少有一個 HPO term 時才會排入 Exomiser/LIRICAL，若 VCF 尚不存在，背景 job 開始前會自動建立或刷新。
...
3. **看變異卡片** — 個案載入後先顯示 SNV/Indel（分段載入），CNV/SV、Mitochondria、STR 與 PGx 在背景載完後補上：
   ...
   - SNV/Indel tier：`1A / 1B / 1C / 2`（互斥）。`1C — Predicted suspect` 保留 ACMG points ≥4，並納入 Core predictors（P-KNN LLR ≥1、AlphaMissense、BayesDel_noAF、Pangolin）及 research predictors（REVEL、SpliceAI）的規劃門檻；predictor 只用於收入 reviewer 候選，不顯示額外 trigger badge，也不修改 ACMG points/class。原本 ClinVar P/LP 0★/conflicting 專屬 tier 已移除，未符合其他 1C 條件者歸入 `2 — Other`。
   - SNV/Indel 卡片的 Score 與各個 in-silico predictor 旁有可 hover/focus 的 `ⓘ`：Score 說明 variant/phenotype 計分公式；predictor 顯示本位點 evidence、PP3/BP4 calibrated threshold 或未校準警語，Reference URL 可直接點開。ClinVar 標題固定為 `ClinVar (2026-07-20)`，內容固定顯示 Nextflow baseline，不放 `ⓘ`；標題旁的外部連結圖示優先用 baseline Variation ID 開啟 NCBI ClinVar，baseline 無紀錄時 fallback 最新版 Variation ID，兩者都沒有時隱藏。只有具臨床意義的 weekly change 才在連結圖示旁放紅色 `↑`／綠色 `↓`，hover 顯示最新版分類、原始 `CLNREVSTAT` review status（含 stars）及最新版日期。ClinVar 與 ACMG 中間有資料時新增 ERepo「classification (derived points)」；點擊後 structured modal 依序比較 Manual、ERepo、GeneBe、In-house，ERepo 顯示 VCEP criteria、Apply 與 ClinGen experts 說明，卡片本身不顯示 panel。顯示順序固定為 P-KNN、AlphaMissense、Pangolin、REVEL、SpliceAI、GPN-MSA、ESM1b、VARITY_R、BayesDel、CADD、DANN、MutPred2、VEST4、PhactBoost、PhyloP、GERP、SIFT、LOFTOOL；至少前六項直接顯示，若 ClinVar／ACMG／LitVar2 欄較高，會再顯示實際可容納的項目，其餘收在 More。高度判斷只計 LitVar2 可見內容，並在初次版面或 LitVar2 尺寸穩定後重新校正。GPN-MSA 是 GRCh38 SNV 的連續相對分數，越低越 deleterious；`≤ -7` 只以紅色 contextual tail 提示，未校準成 ACMG PP3/BP4，也不產生 benign evidence，且不影響 tier、ACMG、filter、排序或報告。CADD、MutPred2、VEST4 與既有已校準工具依文獻 PP3/BP4 threshold 套色；沒有可靠 cutoff 的 DANN 與非 score 型 LOFTOOL 不套色。
   - ClinVar/ACMG 下方顯示 `LitVar2 (版本日期)`：標題維持純文字；有唯一 logical variant 命中時，旁邊的 `SquareArrowOutUpRight` 外部連結圖示開啟代表 LitVar2 結果頁，`No reference`、`NA` 等其餘狀態則開啟 LitVar2 首頁。前五個 PMID 分別直連 PubMed，超過五篇顯示 `and N others`。本地 SQLite v2 保存完整 PMID/identifier aliases；相同 ClinGen CA，或相同 rsID + gene + normalized HGVS（protein HGVS 統一一／三字母格式）的重複 raw records 會合併，PMID 取聯集去重，卡片只顯示一次並可展開 `Merged from N LitVar2 records` 追查每筆來源。合併後仍有多個真正不同的候選才顯示 `Ambiguous match (N variants)`。v1 SQLite 仍可讀，下一次手動／每月更新會沿用已下載的 bulk 重建 v2，不會重複下載相同壓縮檔；schema/lookup version 納入 per-sample cache fingerprint。三級 post-processing 只查 `is_review_retained()` 的 final-review variants（包含 weekly ClinVar 升／降 P/LP rescue），gene search 與卡片 refresh 使用本地按需查詢；舊個案需重跑三級或按 refresh。全程不呼叫 LitVar2 API，也不影響 tier、ACMG、排序或報告。「輸出 PDF」會在移除外部連結前，先把 PMID 與 `and N others` 轉成可換行純文字並保留 merged summary，避免列印結果只剩逗號。
   - 左側「基本資料／報告／分析」及子區導覽點擊後直接跳到目標高度，不再以 smooth scrolling 快速滑過整頁；窄螢幕仍會在跳轉後自動收合側欄。
   ...
   - 主畫面讀取 `08_postprocessing/{LIS_ID}.snv_indel.review.tsv`（舊檔 fallback `snv_indel.review.tsv`）：SNV/Indel 會排除 chrM/MT，WES 先濾 DP < 20，再保留 candidate BED 內且 `GNOMAD_G_AF < 0.01` 或 AF 缺值的位點，並 rescue baseline ClinVar P/LP 及 weekly ClinVar `UP_TO_PLP` / `DOWN_FROM_PLP`。這套最終保留規則由 `snv_review.is_review_retained()` 統一提供給 review materialization、SpliceAI、GeneBe live API fallback 與 LitVar2，避免各 annotation 候選集合漂移。GeneBe、weekly ClinVar comparison、GIAB、院內 AF、MANE、SpliceAI 等 post-processing 差異存於 prefixed `snv_annotations.sqlite`，產生 review、gene search 與 reviewer-marked 補點時才合併；GPN-MSA 則直接以 tabix join compact review TSV 的 SNV，不寫 sparse overlay，因此完整 raw gene search 不帶 GPN-MSA。完整來源固定為 `03_acmg/*.snv_indel.acmg.tsv`，不再永久保存第二份數 GB 的 `snv_indel.annotated.tsv`。gene index offsets 也指向 03_acmg raw；缺 index 時仍可 bounded-memory 串流搜尋。
   ...
   - Manual ACMG modal 上方各來源（有 ERepo 時也包含 ERepo）的 criteria 摘要一律展開 evidence strength，例如 `PM1_Moderate`、`PP3_Supporting`、`PVS1_Very_strong`；Manual 的 Global reusable 行也使用相同格式。這是顯示層格式化，不改資料庫內的精簡 criteria text、Apply 內容或計分。
   ...
   - SNV/Indel 卡片的疾病清單以最新版 `OMIM.xlsx` 的 `Disease1..16` 為權威來源；每個 slot 依 Excel 原順序保留自己的疾病名稱與 rich description / synopsis，即使多個 slots 共用同一 phenotype MIM 也不互相去重或覆寫。若部署了 `NGS_UI_GENE_DISEASE_DB`，後端會把 GenCC / ClinGen / MONDO association 依 gene 合併進 `disease_associations`：supplemental association 仍用 phenotype MIM、MONDO ID、source disease ID 與 gene-scoped normalized disease name 去重；命中 OMIM slot 時只補 evidence badge，不改 OMIM 文字，同一 phenotype MIM 命中多個 OMIM slots 時 evidence 會附加到全部匹配列，且不再額外重複顯示 supplemental disease。只有完全沒有 OMIM match 的 association 才放在下方補充區。DB 由 `python scripts/update_gene_disease_db.py` 建置：下載 GenCC submissions CSV、ClinGen gene validity CSV、MONDO JSON，建立 `gene_disease.sqlite`、audit `gene_disease.tsv`、`source_manifest.json` 與 `build_report.tsv`；GenCC/ClinGen 只納入 `Limited` 以上，低於 `Limited`、disputed/refuted 等會略過。UI 固定先列 OMIM disease，以稍深灰底顯示，第一筆補充疾病前另有細分隔線；OMIM badge 先排除後才限制最多三個 supplemental badges，避免 evidence 被截掉。補充列目前只在 UI 顯示，不改 DOCX 的 `report_diseases` 選取；OMIM disease 只有單行、沒有下方 description/synopsis 時，summary 後會以小 `*` 提醒 curator 補描述，星號不寫回資料也不進列印輸出。只有 `Disease1..16` 全數為空時，後端才會依序從多行 `OMIM_disease` 合成最多 16 個單行 fallback；已有任一 curator slot 時不補其餘空欄，避免 slot 對應錯位。
   - Secondary findings 從 `snv_indel.review.tsv` 產生，依序包含 ACMG SF、遺傳癌症 v2.0（`WES-I__腫瘤醫學__遺傳癌症 v2.0`，209 genes）、中風相關基因（`WGS__神經科__Stroke`）與 Carrier screening，四者共用 ACMG SF 的規則；需符合 panel gene、ClinVar P/LP 或 SNV tier `1A/1B/1C`、VAF ≥ 0.2、zygosity 非 ref，因此會納入 LOFTEE HC、ACMG points ≥4、P-KNN LLR ≥1 及與主 SNV 卡片相同的其他 predictor 候選。分析區保留全部候選點位；同一 variant 命中多個 panel 時會依各 panel 基因內容分別顯示在每個分類，但所有卡片共用同一筆 variant review state，勾選／取消、ACMG、Comment、transcript 與疾病選取會同步。只有 ClinVar P/LP 預設 ✓；其他 1B/1C 候選預設未勾，報告區只顯示 ✓ 點位。任一所屬 panel 有明確取消即以取消為準，健檢匯出時才將不同 panel 的相同 variant 合併去重。
   - 「匯出健檢報告」只提供 ACMG 疾病風險基因、遺傳癌症 v2.0、中風相關基因、帶因者篩查與藥物基因體學五個選項；ACMG 與 PGx 預設勾選，遺傳癌症、中風與帶因者預設不勾。所選疾病 panel 的點位會先合併去重，再統一分成「第一類：與疾病風險相關之致病性或疑似致病性變異位點」與「第二類：符合帶因者狀態之致病性或疑似致病性變異位點」；AD、X-linked、純 AR 同基因兩個以上變異及純 AR homozygous 變異列第一類，純 AR 單一 heterozygous 變異列第二類。§五「本次檢測基因包括」的 panel 標題不再帶「第一類／第二類」等前綴。附錄另起新頁，「附錄」置中且下方空一行，再依序列出變異位點參考資料、ACMG疾病風險基因與相關疾病簡介（含本節文獻），最後才接完整用藥建議。PGx 主文固定列 21 個 CPIC Level A genes，統一為「基因／檢測項目、Allele 1、Allele 2」三欄，不列表型、用藥建議概覽或藥物建議摘要；一般 star allele／haplotype 與已分相 DPYD 各 allele 分欄，DPYD 多筆未定相 source diplotypes 則跨兩個結果欄完整列出所有 named variants、het/hom 並標示「相位未定」。CFTR Reference 採 JSON label 的 `Reference | Reference`，不輸出重複且過長的 ivacaftor-response 功能名稱。ABCG2／VKORC1 則在第一欄列 rsID＋HGVS，後兩欄列 transcript-oriented base 及 Reference／Variant。HLA-A／HLA-B 分型後另列 `HLA-A*31:01`、`HLA-B*15:02`、`HLA-B*57:01`、`HLA-B*58:01`，這四個篩檢子列的第一欄固定縮排兩格，每條 allele 個別顯示 Positive／Negative，因此同型合子風險型可明確顯示兩個 Positive；附錄仍使用 `pharmcat.report.json` 的 CPIC/FDA 完整建議，不含 CYP3A4 與 IFNL3。
   - 健檢匯出選項各自控制主文、基因清單與附錄。未勾 ACMG SF 不輸出其疾病簡介及文獻；未勾 PGx 不輸出基因型、官方用藥資訊、完整用藥建議，也不保留方法中的藥物基因相位／CYP2D6 說明。空白或無效選項回報錯誤，不自動恢復預設勾選。ACMG SF 清單中的「腫瘤相關基因」使用 `ACMG_SF_v3.3` 中的 28 個基因，來源為 `phenotype_data/custom_panels/ACMG_SF_v3.3.txt`；新增的獨立「遺傳癌症 v2.0」選項則使用與診斷相同的完整 209-gene panel，只有勾選才納入其點位與基因清單，單獨勾選不加入 ACMG 衛教或 PGx。
   - ACMG 衛教附錄採「索引表＋疾病介紹」，依血脂、遺傳性腫瘤、心血管、代謝與內分泌、麻醉及其他疾病分六類，完整涵蓋 ACMG SF v3.3 的 84 個基因、36 個疾病群組。依 2026-09-11 使用者追蹤修訂與註解合併三種瘜肉症（APC、MUTYH、BMPR1A、SMAD4），並更新疾病名稱、遺傳模式、臨床表現與追蹤治療文字。2026-09-14 再依追蹤修訂與 4 則註解簡化疾病及追蹤治療文字，三種心肌疾病統一使用「心肌病變」，結節性硬化症及 RPE65 說明改為白話。各疾病間固定空一行；僅 FH 保留補充說明，文獻標題改為「參考資料」並移除查閱日期說明。同日第三輪修訂將 NF2 中文名稱改為「神經纖維瘤症候群第二型」，參考資料接續排版、不強制另頁；前方共留兩個空白行，標題為 11 pt 粗體（同疾病內文），文獻清單 9 pt、網址 8 pt。索引說明緊接「疾病索引」，中英文疾病標題同段同字級；移除遺傳模式通則、逐疾病來源編號及被刪除的補充說明，保留完整 ACMG、ClinGen、GeneReviews／NCI 文末來源與索引跳轉。勾選 ACMG 即附上完整內容，無可回報位點也會加入；不顯示逐基因檢出狀態。內容維護於 `backend/app/report_content/acmg_sf_v3_3.json`；`python scripts/preview_acmg_sf_appendix.py --output /tmp/acmg-appendix.docx` 可產生不讀取病患資料的預覽。
   ...
   - SNV/Indel 卡片的 Zygosity 統一顯示為 `het`／`hom`／`hemi`；`hom` 與 `hemi` 使用和 Likely pathogenic 相同的淺紅底，並相容來源資料的 `homozygous`／`hemizygous` 長寫法。這是主畫面視覺提醒，不改 tier、filter、ACMG 或報告內容。
   ...
   - CNV：`CNV-1A`（Clinical）、`CNV-1B`（Pathogenic）；SV：`SV-2A / SV-2B`。CNV/SV 分析區依影響類型及臨床優先順序排序，報告區依 `max_pheno_score + scaled AnnotSV ranking score` 由高到低排序；基因表預設只顯示 phenotype 相關基因，按小三角形才展開 phenotype score 為 0 的其餘基因。
   - DRAGEN CNV post-processing（policy v2）只用同一 source sample 的 `cnv_sv.vcf.gz` 作為事件與品質來源：擷取 `SVTYPE=CNV`、ALT DEL/DUP、`SVCLAIM=D/DJ`，保留整合後 PASS；另以 Rule B 保留僅 `cnvLength`／`cnvQual` 的 DJ 事件，要求明確斷點連結、同染色體同類型、支持 SV 與 FT（若有）PASS、GT 有 alt 且不衝突、單筆 SV 與 CNV 雙向重疊 ≥50%。不讀原始 `cnv.vcf.gz` 或 raw SV；原始品質不再否決整合後 PASS。全部保留事件依整合後座標重新 AnnotSV，寫入 `08_postprocessing/{LIS_ID}.cnv.review.tsv`，不混入 06 舊註解。可選 `--base-tsv` 只沿用舊事件 ID，缺此檔亦可執行；新事件沿用以 OrigCnvPos/End（缺值用 POS/END）建立的穩定 ID。`cnv_rescue.json` v2 記整合來源 hash、PASS／B 計數與證據；新卡片只呈現整合後資訊，B 不改寫成 PASS。零事件產生空 review，缺整合檔才整個樣本 fallback 既有來源；不新增 rescued-only 註解檔，舊檔由 managed promotion 清理。in-house／SV 頁的註解來源不變；DRAGEN WES 也須有合格整合檔才能使用。舊個案需重新執行 CNV post-processing／三級分析，重新整理頁面不會重算 AnnotSV。
   - 同來源、同染色體、同為 deletion 或同為 duplication 的 CNV/SV，只要相鄰 gap ≤ `250 kb`，或邊界交疊同時 ≤ `10 kb` 且 ≤ 較短片段的 `10%`，就會預設自動整合；比例只用來避免短 CNV 的主要部分互相重疊時被誤當成相鄰片段，不作致病性判斷。copy number 差異不再阻擋視覺整合，原始片段仍可展開查看各自 CN。UI、DOCX 與個案清單改用合併後 parent；parent 會取代最佳原始 segment 的位置，不會掉到 tier 最後。前端會依 sample payload 快取整合結果，避免卡片 render 時反覆重建 parent。
   ...
   - 健檢 DOCX：ACMG SF 維持既有 ASCII 變異表格，表格內 ACMG/AMP 分級保留英文；表後依 AD、AR、X-linked、zygosity、同基因回報變異數及性染色體組成自動產生中文結果意義與建議。性染色體組成優先讀取已複製的 `ploidy.vcf.gz` 之 `estimatedSexKaryotype`，其次使用 EMR 性別，無資料時採中性文字。AR 單一 heterozygous 變異會寫明「本次僅檢出一個符合報告條件之變異，檢測結果符合帶因者狀態」；AR 多變異會寫明兩變異相位尚未確認，建議進行家族成員檢測以釐清是否位於不同等位基因，方能判斷是否符合體染色體隱性疾病之雙等位基因致病型態。建議句使用「遺傳諮詢或門診相關專科」。X-linked 女性單一變異採可能受 X 染色體失活型態及疾病表現範圍影響的保守文字。WGS 健檢報告平均深度為 27X；方法限制在 CNV 未涵蓋範圍後新增短讀長 WGS 可能偵測疏漏，以及非長讀長造成藥物基因 phasing／haplotype 不確定性兩點，後續 CYP2D6 專一 CNV、粒線體與報告聲明順延為第 7–9 點；診斷報告的 WGS 深度仍為 30X 且不受影響。PGx 報告與主畫面共用同一套後端 presenter；健檢 DOCX 主文使用獨立 `health_genotype_rows`，只輸出固定 21-gene 的「基因／檢測項目、Allele 1、Allele 2」表格與官方查詢連結，不輸出表型、用藥概覽、藥物摘要或動態藥物數量敘述。一般 star allele／haplotype、已分相 DPYD 與 HLA 完整分型逐 allele 分欄；DPYD 多筆未定相結果跨兩個 allele 欄列出全部變異與 het/hom，明示「相位未定」且不假造 cis/trans。CFTR Reference 顯示為 `Reference | Reference`。ABCG2 顯示 `rs2231142 (c.421C>A)`、VKORC1 顯示 `rs9923231 (c.-1639G>A)`，後兩欄統一列 transcript-oriented base 與 Reference／Variant。HLA 另緊接列出 `HLA-A*31:01`、`HLA-B*15:02`、`HLA-B*57:01`、`HLA-B*58:01` 的逐 allele Positive／Negative，四個篩檢子列的第一欄縮排兩格；同型合子風險型顯示 `Positive | Positive`，aggregate positive 在缺少完整分型時不推定兩個 copies，無可判讀結果顯示 `No Result`。MT-RNR1 的 Allele 2 固定為 `N/A`，G6PD 來源只有單一 allele 時亦用 `N/A`，避免複製成假性雙套結果。完整 CPIC/FDA 建議仍依藥物 A-Z 置於附錄，同一藥物只列一次且 CPIC/FDA 來源放在建議句尾括號；只有 MT-RNR1 在 JSON 無有效結果時可由 TSV 補值。IFNL3 不會出現在 PGx 主文、附錄或基因清單，標題與參考依據不列 FDA。
   - 健檢 ACMG SF 的附錄參考資料順序跟上方點位顯示順序一致；同一基因多位點在表格合併時，參考資料也連續列出。
   - ROH 一律只在三級 staging 的 `08_postprocessing` 處理，不修改既有 `00`–`07`。in-house 先找二級 `08_roh/{source}.HomRegions.tsv` 並優先使用；缺檔且勾 Research-only 時，才以 `04_snv_indel/{source}.haplotypecaller.vcf.gz` 和 AutoMap 1.3 container 補跑，補跑失敗會留下 warning 並退回 `{source}.roh.txt`；未勾 Research-only 直接使用 BCFtools。DRAGEN 固定精確讀 `<run>/other/{source}/germline_seq/{source}.roh.bed` 與 metrics，不使用 AutoMap。三種來源都轉成 0-based half-open 的 prefixed `roh_regions.tsv`，UI 以 1-based inclusive 顯示；SNV 的 `In ROH` 由預設可信區段動態 join。預設門檻為 AutoMap 原生 ≥1 Mb、BCFtools ≥1 Mb/markers ≥25/Q ≥20、DRAGEN large ROH ≥3 Mb；畫面仍可查看全部與其他長度門檻，且 DRAGEN 的 `0.070` 顯示為 `0.070%`，不解讀成 7%。
...
PGx 原始輸出可用唯讀工具批次稽核 JSON／TSV coverage、固定 21-gene 表格 projection、實際 allele 組合、HLA risk allele copy 狀態、No Result，以及 DPYD 多筆 source diplotypes／未定相 variant calls；工具只把稽核檔寫到指定的 output directory：

```bash
python3 scripts/audit_pgx_report_inputs.py \
  --root /home/datalake_Intermediate/pipeline/tertiary_output \
  --prefix 26T \
  --output-dir /tmp/pgx-audit-26T
```

SNV/Indel 卡片的 ESM1b 依 ClinGen SVI 校準區間上色；ESM1b 分數越低越偏致病，多 transcript TSV 會取最低分作為 worst case。
5. **匯出報告** — 「匯出診斷報告」下載 `GET /api/samples/{LIS}/report.docx`；DOCX 依序輸出第一類、第二類、固定建議文字，再集中列出各 variant 的參考資料。同一類別內的同基因 SNV/Indel 會合併為一個標題與一張多列表格，重複疾病說明只列一次；若各點 ACMG 分級不同，表後中文說明會同時列出所有分級，例如「致病性及疑似致病性」。第一類沒有結果時固定輸出「未找到與臨床症狀相關基因之已知致病性變異位點。」CNV/SV gene 表的 Phenotype 優先使用 `OMIM.xlsx`，每個 OMIM disease 與已知致病區域重疊 disease 都可勾選；DOCX 在原本 Disease 位置輸出「OMIM 勾選＋致病區域勾選＋自由文字」的去重聯集，以「、」連接且不增加段落。多基因或無 OMIM gene 的片段仍把這個聯集接在第 1 點位置描述。未涵蓋 OMIM 疾病相關基因時不再列出一般基因清單。SNV/Indel 會帶 TSV 的 `RS_ID`，RS ID 欄會預留尾端空格；ClinVar 欄也預留尾端空格，避免 `conflicting classifications` 與 ACMG 欄黏在一起；全形破折號按兩個顯示欄寬計算，避免後方多出一格。SNV transcript header 若有 RefSeq 對到 Ensembl，會顯示 `GENE (ENST...; NM_...)`，沒有 RefSeq 則只顯示 Ensembl；SNV/Indel 與 Mito 核苷酸欄每行最多 13 字元且蛋白質括號另起一行。§五.4 grouped gene list 的 HPO/panel 區塊之間會留空行，且只列 expanded reportable disease-associated genes；WES/WGS DOCX 都不輸出 dead-zone exon 標註，dead-zone 僅保留在主畫面與 phenotype 工具查詢。旁邊的「輸出 PDF」會開啟列印視窗，輸出報告區的 causative / other / candidate 卡片摘要與本次檢測基因清單；PDF 與 DOCX 一樣會先詢問基因清單要按 HPO/panel 分組或全部合併去重，並在按下輸出時才呼叫 `/api/samples/{id}/report-gene-list` 載入基因清單。列印版會略過 comment、More、Secondary findings 與 CNV/SV overlap 明細。
...
二級分析 modal 由右上角「二級分析」開啟，後端掃描 WES roots（NextSeq2000 與 Reanalysis）與 WGS Novaseq FASTQ，提供 WES/WGS 兩個 typeahead 搜尋框、更新索引、批次清單與「加入同批全部樣本」。WGS 下拉選單依主 sample 聚合顯示，每個 sample 註明 lane 數與 FASTQ 檔案數；只收 `*_S*_L00*_R[12]_001.fastq.gz`，不使用同資料夾內的 merged FASTQ。建立 samplesheet 時，後端會把每個 WGS sample 展開成每 lane 一列並寫入 `lane` 欄，交由 FASTP 分 lane QC、FQ2BAM 合併；WES 的掃描、顯示與 samplesheet 行為不變。按「建立 sample sheet」後會先在 `NGS_UI_SECONDARY_SAMPLESHEET_STAGING_ROOT/<batch_name>/samplesheet.csv` 寫出 CSV；DGM `/home/datalake_Raw/...` 會映射到 DGX2 `NGS_UI_SECONDARY_DGX_RAW_ROOT/...`（預設 `/datalake_Raw/datalake_Raw/...`），已經位於 `/datalake_Raw/...` 的路徑不重寫。產生的 tmux launch block 會在 source DGX2 環境後設定 `umask 0002`，再由 runner 建立 `OUT_DIR`、`LAUNCH_DIR`、`WORK_DIR` 並複製 staged samplesheet，讓同屬 `dgm_nckuh` 的使用者可共同管理新建的 Nextflow work tree；共享 root `/raid/DGM/work` 應設定為 group `dgm_nckuh`、mode `2775`。Nextflow 結束或失敗時會複製 `${LAUNCH_DIR}/.nextflow.log` 到 `${OUT_DIR}/nextflow.log`，並保留 tmux pane 不自動關閉，方便確認最後狀態。產生的 Nextflow 指令依 seq type 分流：WES 使用 `dgx` profile，WGS 使用 `dgx_single` profile；WES 另加 `--run_gcnv true`，WES/WGS 都預設加入 `--run_manta`、`--run_expansionhunter`、`--run_automap`，讓二級分析直接產出三個模組的結果。Reanalysis FASTQ 也直接指向原始 `*.R1/R2.clean.fastq.gz`，不再複製或建立 symlink；只改 samplesheet 的 `sample` 欄。因 NGS-UI 跑在 DGM，不能直接清 DGX2；modal 的「顯示 DGX2 清理指令」會依 `NGS_UI_SECONDARY_DGX_WORK_ROOT`（預設 `/raid/DGM/work`）產生 guarded shell script，必須複製到 DGX2 terminal 執行。script 會先阻擋仍有 Nextflow process 的狀況、列出待刪項目並要求確認；只有刪除成功且再次確認 root 為空才顯示「已清理」，否則列出殘留並回報失敗。「複製」會直接寫入剪貼簿，plain-HTTP intranet 自動使用隱藏文字框 fallback，不再跳出手動複製視窗。清理後既有批次不能再用 `-resume`。

三級分析 modal 的 in-house 與 DRAGEN VCF 各自使用單一 typeahead；Sample ID 列在「加入批次」左側另有「加入同目錄全部檢體」，會把 DRAGEN 同一 run/input 目錄或 NCKUH 同一上層 batch 目錄的檢體依預設 `{source}-dragen` / `{source}-nckuh` ID 一次加入，並沿用來源限制與 sample/path 去重。兩者共用的 discovery index 由 `ngs-ui-tertiary-index-update.timer` 每天台北時間 02:00 自動更新，人工「更新索引」與 24 小時 stale fallback 仍保留，跨程序刷新會用 file lock 避免同時寫入。PGx 和「更新索引」之間的「更新 LitVar2」會啟動 detached worker，背景下載官方 bulk（壓縮檔約 1.8 GB）並重建本地 SQLite；新資料先驗證再原子切換，失敗繼續使用舊版，modal 會顯示進度、版本與錯誤。每次送出三級分析都會啟動 Nextflow `-resume`，不再由 UI 依正式資料夾中的檔案決定是否略過；DRAGEN 與 NCKUH 各自固定使用一條共享 launch/session/work lineage，因此同一 sample 即使前後放在不同 batch，只要 Nextflow 判定 task 的 input、script、container 與其他 hash 條件相同，就能沿用 cache，條件改變的 task 則自行重跑。同一 pipeline 類型一次只允許一個 Nextflow process 開啟共享 cache；後送 job 會停在 `waiting-nextflow-cache`，DRAGEN 與 NCKUH 彼此不互鎖，Nextflow 結束後的驗證與 post-processing 也可繼續並行。共享 lineage 第一次建立時會從舊 launch contexts 選擇與本次 sample 重疊最多的可用 session 承接；不同 session ID 的舊 cache 無法合併，未承接到的 task 會由 Nextflow 正常重算並逐步累積到共享 lineage。相同 UI/source sample 正在執行時，重複送出仍會回 HTTP 409。

Nextflow 的 `--out_dir` 指向 job-private `.staging/{job_id}/`，不直接覆寫正式個案。結束後先驗證該 source sample 的 `00_prepare`–`06_cnv_sv` 目錄以及 82 欄 v3.8 SNV、Mito、STR、CNV、SV 產物；SNV 必須包含 `HAPLOID_HET`，所有欄位依名稱解析（不寫死 `ACMG_CLASS` 欄號），並核對 `DBNSFP_VERSION`。`CALLERS=NONE` 會在所有 GUI raw-row consumer 再防禦性排除；男性 chrX 非 PAR 的 `HAPLOID_HET` 則保留並在卡片顯示人工複核警示。有勾 PGx 時再要求 `07_pgx` 與 PGx/PharmCAT 產物。`Research-only` 勾選時 Nextflow 加 `--academic_dbnsfp true`（5.3a），否則維持 4.9c；同一 checkbox 也控制 post-processing 是否補 SpliceAI。每次 Nextflow 成功、失敗或終止時，worker 會在釋放同模式 shared cache lock 前，把 shared launch 的 `.nextflow.log` 原子快照到該 job 目錄，避免後續 batch 輪替 log；三級分析清單的「查看 Log」顯示完整 NGS-UI `log.txt`，旁邊可在 Nextflow 結束後下載該次完整 `.nextflow.log`。執行中的 job 尚未提供不完整下載；改版前沒有快照的舊 job也不會以共享目錄的其他 run 冒充。驗證通過後，SNV post-processing 在 staging 中完成 weekly ClinVar comparison、GeneBe、SpliceAI-only、GIAB、院內 AF、MANE、LitVar2、prefixed overlay/review/index、MITOMAP 與 ploidy 衍生檔；GPN-MSA 不受 Research-only 控制，所有 case 在 review TSV materialization 時都嘗試由固定 GRCh38 pre-computed BGZF/TBI 補分。缺 DB、`.tbi`、tabix 或查詢失敗時，review TSV 保持可用，job log 顯示 warning，並在 `{LIS_ID}.snv_indel.review.tsv.source.json` 的 `gpn_msa_annotation.status` 記錄 `skipped_missing_db`、`skipped_missing_index`、`skipped_missing_tabix` 或 `failed`，不讓三級 job 失敗；成功為 `complete`。SpliceAI、GeneBe live API 與 LitVar2 僅處理共同 final-review predicate 留下的 variants；LitVar2 DB 缺失或查詢失敗仍為 warning/no-op。hidden working TSV 仍在 `finally` 刪除。最後進入 `promote-output`，以可 rollback 的 rename 切換正式 00–07 與 worker-owned 08 衍生檔，保留當下最新的 reviewer metadata、case summary 與 analyses，並最後才切換 `{LIS_ID}.layout.json`。任何 Nextflow、完整性驗證或其他 post-processing 失敗都只清理 staging，原本正式結果與已載入個案維持可用；批次切換中途失敗則還原同批已切換的 sample。若取消 PGx，舊 `07_pgx` 會在成功切換時移除，避免顯示上一次的 PGx 結果。三級分析清單會忽略 `.staging` / `.rollback`；完整刪除仍會清掉對應 pipeline/UI tree 與 job log，執行中的 sample 不可刪。

同一個 `Research-only` checkbox 也控制 in-house 缺少既有 AutoMap 時是否補跑 AutoMap；DRAGEN ROH 不受此設定影響。ROH 正規化、來源副本、summary 與 AutoMap PDF 都是 worker-owned 08 衍生檔，和 overlay/review/index 一起先在 staging 完成、最後才 promotion，因此 AutoMap 不會修改二級或三級的 `00`–`07`。
...
dbNSFP 完全由 Nextflow v3.6 負責：預設 4.9c；勾 `Research-only` 時用 5.3a 並提供 REVEL、MutPred2、VEST4、CADD_phred。Post-processing 的 `annotate_spliceai.py` 只讀 `$NGS_UI_HOME/biotools/spliceai/spliceai_scores.raw.{snv,indel}.hg38.vcf.gz` 及各自 `.tbi`，且只對 final-review variants 補 `SPLICEAI_MAX`，不載入 dbNSFP；未勾 Research-only 時整步略過，勾選但 reference/index 缺失則明確失敗。GPN-MSA 是另一條獨立、非 Research-only 的 final-review annotation：先執行 `scripts/download_gpn_msa_scores.sh` 將官方約 9 billion SNV pre-computed score BGZF/TBI 部署到固定路徑，之後每個 case 以 tabix best-effort join，不設自動更新 timer 或 release hierarchy；可用 `scripts/backfill_gpn_msa.py` 對既有 case 補分。[官方 Hugging Face dataset card](https://huggingface.co/datasets/songlab/gpn-msa-hg38-scores) 標示 MIT license，可商用但仍應保留來源與引用；論文頁面的出版授權是另一件事。最新版 ClinVar 由 `ngs-ui-clinvar-update.timer` 每週三台北時間 04:00 下載官方 GRCh38 VCF、驗證並原子重建 `$NGS_UI_HOME/biotools/clinvar_latest/clinvar_latest.sqlite`；三級 post-processing 對整份 working TSV 串流比對，將最新分類、review status 與日期分開保存在 UI overlay，marker 寫入 `{LIS_ID}.clinvar_comparison.json`。最新版不覆寫 baseline ClinVar 或 tier，只驅動箭頭 hover；診斷／健檢 DOCX 的日期、ClinVar 值及健檢候選集合固定使用 Nextflow 20260720 基準。安裝 timer：`scripts/install_clinvar_update_timer.sh`。
...
# 所有可解析的 case
python scripts/backfill_gpn_msa.py --all

# 需要時可覆寫 root 或 DB
python scripts/backfill_gpn_msa.py --root /path/to/tertiary_output \
  --gpn-msa-db /path/to/scores.tsv.bgz SAMPLE_ID
```
...
三級分析進度條依實測 DRAGEN / in-house batch log 配重：等待同模式共享 cache 鎖時停在 `waiting-nextflow-cache`（2%）；取得鎖後 Nextflow 佔主要時間，但 `queued` 事件只寫 timing log，不推進 UI 百分比；實際 `start/done` 才依 process 權重更新。前處理、mito、STR、prepare CNV/SV 等快速步驟只佔少量進度，`ANNOTSV_SV`、VEP、Pangolin、parse CSQ、ACMG/PGx 等耗時步驟佔主要權重。Nextflow 在 batch channel 尚未收齊時會暫時顯示動態 `1 of 1`、`2 of 2`；後端以本批 sample 數作為進度分母，讓 VEP 等長步驟按完成 case 數平滑前進，不會在第一個 case 就誤判整個 process 完成。modal 會在 `nextflow` 後顯示目前 process 與 `done/total`。Nextflow 結束約落在 82%，post-processing 依目前第幾個 sample 與子步驟推進到 99%，`promote-output` 維持 99%，切換完成才到 100%。Nextflow batch 會同時更新多個 process，因此後端寫入 `nextflow_progress_pct` 時會保留目前最大值，避免較早 process 晚更新造成進度條倒退。首頁會定期查詢後端 active job，因此其他瀏覽器或電腦登入後也會看到正在跑的三級分析；systemd unit 需設定 `KillMode=process`，避免重啟 web service 時把同一個 cgroup 裡的 Nextflow worker 一起 SIGTERM。若 persisted job state 仍是 `queued`/`running`，但 worker PID 已不存在或在 Linux 上已成 zombie，後端讀取 job 時會自動標記為 failed，避免 UI 狀態列永久停在最後百分比。進度面板提供「終止」按鈕，會呼叫後端取消該 job 並向 worker process group 送出終止訊號。三級分析清單右上角提供「清理 Nextflow 暫存」，可清空 `NGS_UI_TERTIARY_NF_WORK_ROOT`（預設 `$NGS_UI_HOME/nf_work`，dev 機為 `/home/n102968/NGS_UI/nf_work`）底下的共享 work 與 `contexts/` cache；若仍有 queued/running job，後端會拒絕清理。

CNV／SV 四個分頁（Clinical／Pathogenic）皆支援影響類型勾選，適用 DRAGEN／NCKUH、WGS／WES：預設顯示「外顯子／剪接影響」和「位置註解不足」，勾選可擴大顯示。篩選列採單行半透明底色，以英文 Exonic / splicing、Only UTR / intronic、Insufficient annotation 顯示各類別數／總數，不另設標題、按鈕或說明列。Clinical 依命中 HPO／panel 的基因分類，Pathogenic 依所有基因；Clinical 的 P／LP 保持可見；Pathogenic 則實際套用勾選篩選，兩區都保留已標記 1／2／C 的事件。分析排序依 P／LP、影響類型、獨立 HPO 符合度、方向一致的 dosage 證據與 AnnotSV 原始分數；卡片列出英文影響類別、臨床相關基因與英文括號描述，以及 HPO match 分數 / 100。這些選項只控制分析畫面，保留原始事件、ACMG、Clinical／Pathogenic 分類與報告選取。缺少必要註解，或 INV／BND 等無法從區間判斷斷點影響時，保留在位置註解不足。更新服務並重新載入個案即可使用，無須為此重跑三級分析。
...
主畫面的 Patient phenotype card 也提供 `WES-I / WES-II / WGS / Other panel` tabs，預設展開 `Other panel`；固定 panel 可直接點 chip 選取，其他 panel 仍可用 typeahead 搜尋。Patient phenotype 與「載入新個案」的 HPO typeahead 共用 `/api/hpo/search`；後端解析 `hp.obo` 的 `is_a` 上一級關係，並在原本 exact／prefix／substring 與 synonym 搜尋後加入拼字容錯，兩處下拉共用同一個 parent-row renderer。
...
- 首頁歡迎文字與版本紀錄放在 `frontend/VERSION.md`；首頁中間的橫向 NGS 分析流程由 `frontend/ANALYSIS_FLOW.md` 管理。流程檔使用固定的 `##` section 與 `|` 前 key 決定版面位置，一般文字修改只需編輯 `|` 後內容。SNV/Indel 的 `review_filter` 文字會顯示在 Prioritization 左側，經箭頭連到右側 `Category`，並直向列出 `1A / 1B / 1C / 2 - Other`；條件或 tier 說明都可直接修改各 key 的 `|` 後內容。前端在未載入個案時依序顯示歡迎訊息、流程圖與版本紀錄；流程檔缺失或格式不完整時只隱藏流程區。之後若有影響判讀流程、報告輸出、資料載入或主要工具入口的更新，需評估是否同步更新這兩份檔案。
