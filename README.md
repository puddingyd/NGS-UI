...
3. **看變異卡片** — 個案載入後先顯示 SNV/Indel（分段載入），CNV/SV、Mitochondria、STR 與 PGx 在背景載完後補上：
   ...
   - 「匯出健檢報告」會先跳出選擇畫面，預設勾 ACMG SF 與藥物基因體學，血脂/腫瘤/中風/Carrier/Proactive 預設不勾。DOCX 保留原診斷報告名稱及章節骨架、固定寬度 `====` ASCII 表格、表格下方列點、檢測方法、注意事項與基因清單；檢驗套組會依 UI 勾選自動組合「可採取醫療處置之疾病風險基因」與「藥物基因體學」。ACMG SF 固定警語說明 v3.3、ClinVar 版本及 P/LP 回報範圍；同基因多位點合併在同一表格並共用一組綜合說明，EXON 為 `.` 等缺值時會改讀 INTRON，完整檢測限制只在 ACMG SF 全段末尾輸出一次。健檢 PGx 內容完全取自 `pharmcat.report.json`：重點摘要以固定寬度 `====` 表格列出「藥物 / 基因 / 建議處置 / 建議依據及等級」，一個藥物一列並依藥物英文名稱 A-Z 排序；接著列固定 22 個 CPIC Level A genes 的基因型與表現型，再依 one-sentence 建議處置彙整藥物分類。完整用藥建議改為 drug-centric 表格，只列 actionable 藥物，並將同一藥物的 CPIC、FDA Label 與 FDA PGx Association 建議合併在同一列；同一 drug/source/action 有 CPIC Strong/Moderate 時保留 Strong。報告基因範圍固定為目前 22 個至少具有一組 CPIC Level A gene–drug pair 的基因，不含 CYP3A4。
   ...
   - 健檢 DOCX：ACMG SF 維持既有 ASCII 變異表格，表後依 AD、AR、X-linked、zygosity、同基因回報變異數及性染色體組成自動產生結果意義與建議；性染色體組成優先讀取已複製的 `ploidy.vcf.gz` 之 `estimatedSexKaryotype`，其次使用 EMR 性別，無資料時採中性文字。X-linked 女性採可能無症狀或因疾病特性及 X 染色體失活而出現臨床表現的保守文字。PGx 報告不再讀 `pgx.tsv`，基因型、表型、重點摘要、用藥建議分類與完整 CPIC/FDA 建議全部由 PharmCAT JSON 產生；重點摘要與完整用藥建議皆以藥物 A-Z 呈現，完整建議同一藥物只列一次。末尾固定列 22 個 CPIC Level A genes，標題與參考依據不再列 FDA。
