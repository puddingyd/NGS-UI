...
| `NGS_UI_TERTIARY_NF_WORK_ROOT` | `$NGS_UI_HOME/nf_work` | 三級分析 Nextflow cache/work root；單一 sample 保留既有 `{sample}/` work，相同 sample set 的 launch metadata 放在 `contexts/` |
...
三級分析 modal 的 in-house 與 DRAGEN VCF 各自使用單一 typeahead。每次送出都會啟動 Nextflow `-resume`，不再由 UI 依正式資料夾中的檔案決定是否略過；Nextflow 在相同 sample set 的獨立 launch context 內自行判斷哪些 task 可沿用、哪些要重跑。單一 sample 第一次改用新 context 時會嘗試匯入舊共用 launch 目錄中「相同 work directory、最近一次成功」的 session cache，既有 `{sample}/` work directory 也維持不變；無相符或 cache 已清除時就由 Nextflow 正常重算。相同 UI/source sample 正在執行時，重複送出會回 HTTP 409。
