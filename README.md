...
3. **看變異卡片** — 個案載入後先顯示 SNV/Indel（分段載入），CNV/SV、Mitochondria、STR 與 PGx 在背景載完後補上：
   ...
   - 健檢匯出選項各自控制主文、基因清單與附錄。未勾 ACMG SF 不輸出其疾病簡介及文獻；未勾 PGx 不輸出基因型、官方用藥資訊、完整用藥建議，也不保留方法中的藥物基因相位／CYP2D6 說明。空白或無效選項回報錯誤，不自動恢復預設勾選。健檢的「腫瘤相關基因」使用 `ACMG_SF_v3.3` 中的 28 個基因，來源為 `phenotype_data/custom_panels/ACMG_SF_v3.3.txt`；診斷用「遺傳癌症 v2.0」是另一套 panel。
