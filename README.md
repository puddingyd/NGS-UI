...
CNV／SV 四個分頁（Clinical／Pathogenic）皆支援影響類型勾選，適用 DRAGEN／NCKUH、WGS／WES：預設顯示「外顯子／剪接影響」和「位置註解不足」，勾選「僅 UTR／內含子」可擴大顯示，也可一鍵顯示全部／恢復預設。Clinical 依命中 HPO／panel 的基因分類，Pathogenic 依所有基因；Clinical 的 P／LP 保持可見；Pathogenic 則實際套用勾選篩選，兩區都保留已標記 1／2／C 的事件。分析排序依 P／LP、影響類型、獨立 HPO 符合度、方向一致的 dosage 證據與 AnnotSV 原始分數；卡片列出具體基因／影響原因。這些選項只控制分析畫面，保留原始事件、ACMG、Clinical／Pathogenic 分類與報告選取。缺少必要註解，或 INV／BND 等無法從區間判斷斷點影響時，保留在位置註解不足。更新服務並重新載入個案即可使用，無須為此重跑三級分析。
