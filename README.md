...
3. **看變異卡片** — 個案載入後先顯示 SNV/Indel（分段載入），CNV/SV、Mitochondria、STR 與 PGx 在背景載完後補上：
   ...
   - 健檢 DOCX：ACMG SF 維持既有 ASCII 變異表格，表後依 AD、AR、X-linked、zygosity、同基因回報變異數及性染色體組成自動產生結果意義與建議；性染色體組成優先讀取已複製的 `ploidy.vcf.gz` 之 `estimatedSexKaryotype`，其次使用 EMR 性別，無資料時採中性文字。PGx 先輸出 CPIC Strong/Moderate 與 FDA therapeutic-management 摘要，高風險基因－藥物組合逐條提醒，其餘聚合後保留於完整 CPIC 建議。
