...
成大醫院基因醫學部的 NGS 三級分析判讀工具。次級 pipeline（Nextflow，跑在另一台 compute cluster）產出 per-sample 的註解 TSV，本平台讓 reviewer 載入個案、檢視 SNV/Indel + CNV/SV + Mitochondria + STR + PGx 變異、標記 causative / candidate / other、整理 ACMG SF / 血脂 / 腫瘤 / 中風 / proactive / carrier secondary findings、撰寫判讀意見，並匯出診斷報告或健檢報告 (docx)。另附一個獨立的「臨床表徵輸入 (HPO / gene panel)」工具掛在 `/phenotype/`。
...
| `NGS_UI_SNV_CACHE_MAX_RAW_MB` | `100` | 只有小於此大小的完整 `snv_indel.annotated.tsv` fallback 解析結果才可進 SNV cache；gene search 缺 index 時會串流掃描 raw TSV，不會整份物件化 |
...
3. **看變異卡片** — 個案載入後先顯示 SNV/Indel（分段載入），CNV/SV、Mitochondria、STR 與 PGx 在背景載完後補上：
   ...
   - 「匯出健檢報告」會先跳出選擇畫面，預設勾 ACMG SF 與藥物基因體學，血脂/腫瘤/中風/Carrier/Proactive 預設不勾。DOCX 保留原診斷報告名稱及章節骨架、固定寬度 `====` ASCII 表格、表格下方列點、檢測方法、注意事項與基因清單；檢驗套組會依 UI 勾選自動組合「可採取醫療處置之疾病風險基因」與「藥物基因體學」。ACMG SF 固定警語說明 v3.3、ClinVar 版本及 P/LP 回報範圍；同基因多位點合併在同一表格並共用一組綜合說明，EXON 為 `.` 等缺值時會改讀 INTRON，ACMG 參考資料移到 ACMG SF 區塊尾端並置於檢測限制警語之前，完整檢測限制只在 ACMG SF 全段末尾輸出一次。健檢 PGx 內容完全取自 `pharmcat.report.json`：重點摘要以固定寬度 `====` 表格列出「藥物 / 基因 / 建議處置 / 建議依據及等級」，一個藥物一列並依藥物英文名稱 A-Z 排序，且只列 CPIC Strong/Moderate 與 FDA therapeutic-management，FDA-label-only 藥物只留在完整建議；接著列固定 22 個 CPIC Level A genes 的基因型與表現型，phenotype 欄優先使用 JSON phenotype，`n/a` 或空值時改用 allele function，兩者皆無時顯示 `No phenotype assigned`；MT-RNR1 在 JSON 無有效結果時可由 `pgx.tsv` 補基因型/表型，再依 one-sentence 建議處置彙整藥物分類。完整用藥建議改為 drug-centric 表格，只列 actionable 藥物，表格總寬與基因型表格對齊，並將同一藥物的 CPIC、FDA Label 與 FDA PGx Association 建議合併在同一列；同一 drug/source/action 有 CPIC Strong/Moderate 時保留 Strong，CPIC/FDA 來源皆改放在各段 recommendation 句尾括號，FDA therapeutic-management 內容先於 FDA label 內容，並清理 PharmCAT/FDA label 原文的引號與省略符號。報告基因範圍固定為目前 22 個至少具有一組 CPIC Level A gene–drug pair 的基因，不含 CYP3A4。
   ...
   - PGx / PharmCAT：讀取 pipeline `07_pgx/{sample}.pgx.tsv` 與精簡後的 `pharmcat.report.json`，依 gene 聚合 diplotype、phenotype、activity score 與 drug recommendations，分成 Clinically actionable、Routine / negative screens 與 Additional PharmCAT genes；另保留精簡的 CPIC、DPWG、FDA label 與 FDA PGx association annotations，完整 PharmCAT JSON 不放進核心樣本 payload。
   - DRAGEN 三級分析會依原始 hard-filtered VCF basename，從同層精確尋找對應的 `*.ploidy.vcf.gz`，複製為 `tertiary_output/{LIS_ID}/ploidy.vcf.gz`；nckuh pipeline 不執行此步驟。基本資料的性別欄以 M↔XY、F↔XX 比對，一致顯示綠底，不一致或 `XXY`、`X` 等其他 karyotype 顯示紅底及 ploidy call。
   - 健檢 DOCX：ACMG SF 維持既有 ASCII 變異表格，表後依 AD、AR、X-linked、zygosity、同基因回報變異數及性染色體組成自動產生結果意義與建議；性染色體組成優先讀取已複製的 `ploidy.vcf.gz` 之 `estimatedSexKaryotype`，其次使用 EMR 性別，無資料時採中性文字。X-linked 女性採可能無症狀或因疾病特性及 X 染色體失活而出現臨床表現的保守文字。PGx 報告不再以 `pgx.tsv` 產生建議文字，基因型、表型、重點摘要、用藥建議分類與完整 CPIC/FDA 建議主要由 PharmCAT JSON 產生；MT-RNR1 基因型/表型在 JSON 無有效結果時可從 TSV 補值。重點摘要與完整用藥建議皆以藥物 A-Z 呈現，重點摘要不列 FDA-label-only 項目，完整建議同一藥物只列一次且 CPIC/FDA 來源放在建議句尾括號。末尾固定列 22 個 CPIC Level A genes，標題與參考依據不再列 FDA。
...
二級分析 modal 由右上角「二級分析」開啟，後端掃描 WES roots（NextSeq2000 與 Reanalysis）與 WGS Novaseq FASTQ，提供 WES/WGS 兩個 typeahead 搜尋框、更新索引、批次清單與「加入同批全部樣本」。WGS 一律列 lane FASTQ（`*_S*_L00*_R[12]_001.fastq.gz`），不使用 merged FASTQ；建立 samplesheet 時會加 `lane` 欄。按「建立 sample sheet」後會先在 `NGS_UI_SECONDARY_SAMPLESHEET_STAGING_ROOT/<batch_name>/samplesheet.csv` 寫出 CSV，FASTQ 路徑會轉成 DGX2 可見路徑（移除最前面的 `/home`）；產生的 tmux launch block 會由 DGX2 runner 建立 `OUT_DIR`，再把 staged samplesheet 複製到 `OUT_DIR/samplesheet.csv` 後啟動 Nextflow，避免 NGS-UI 建立的 output dir owner 造成權限問題。Nextflow 結束或失敗時會複製 `${LAUNCH_DIR}/.nextflow.log` 到 `${OUT_DIR}/nextflow.log`，並保留 tmux pane 不自動關閉，方便確認最後狀態。單樣本使用 `dgx_single` profile，多樣本使用 `dgx`。Reanalysis FASTQ 也直接指向原始 `*.R1/R2.clean.fastq.gz`，不再複製或建立 symlink；只改 samplesheet 的 `sample` 欄。

三級分析 modal 的 in-house 與 DRAGEN VCF 各自使用單一 typeahead 輸入格；點入輸入格即展開全部 VCF，輸入 sample / run / path 後即時縮小候選清單，候選列顯示 sample、run、檔案大小與建立時間；選定後輸入框只保留 sample name，完整資訊留在 title。DRAGEN VCF 預設同時掃 `/home/datalake_Raw/Novaseq` 與 `/home/datalake_Intermediate/n102968`（可用 `NGS_UI_DRAGEN_VCF_ROOTS` 以 `:` 分隔覆寫）。worker 會保留原始 `source_sample_id`，並為 v3.x pipeline 產生 sample sheet 後執行 `nextflow ... --samplesheet ... --pipeline_type nckuh|dragen --out_dir /home/pipeline/tertiary_output -resume`。pipeline 完成後，NGS-UI 逐筆複製 SNV/Mito/CNV/SV/STR/PGx sidecar 並寫 `pipeline_source.json`；SNV TSV validation 會檢查核心 transcript 與 ACMG 欄位，且同時相容 v3.1-v3.4 的 65 欄與 v3.5 的 64 欄格式。post-processing 會在 review TSV/gene index 前用 `scripts/annotate_mane_refseq.py` 讀 `NGS_UI_MANE_SUMMARY`（預設 `$HOME/NGS_UI/biotools/MANE.GRCh38.v1.5.summary.txt.gz`）補 `REFSEQ_NUC/REFSEQ_PROT/MANE_STATUS`。若 pipeline CNV/SV 兩檔都複製成功，post-processing 會跳過本機 AnnotSV fallback，缺任一檔才保留 fallback。執行進度以 step-based 進度條顯示，Nextflow 會從 stdout 追蹤 PREPARE_VCF / VEP / Pangolin / CSQ parse / ACMG 等內部 process；完成事件不再插入獨立 `[nextflow-step]` 或 `[step] nextflow:*` 行，而是在原本的 Nextflow process 行尾補上 `elapsed=...m [YYYY-MM-DD HH:MM:SS]`。post-processing 只追蹤實際執行的 GeneBe、extra VEP、AnnotSV，review TSV 與 gene index 則列為 sample 預建步驟。詳細 log 預設收合，跳過的 Extra VEP / AnnotSV 不留下空 section；Nextflow process history 仍另存於 `state.json.nextflow_step_history`，`step_history` 則保留 UI 進度 milestone。「三級分析清單」會合併 `/home/pipeline/tertiary_output/` 與 NGS-UI job state，右上角搜尋框可依 sample/source ID 即時篩選；失敗且尚未建立 output 的 sample 仍會顯示 log。刪除時會一併刪除 pipeline output、`$NGS_UI_HOME/tertiary_output/{sample}/` 與該 sample 的 NGS-UI job log 目錄，執行中的 sample 不可刪除。新 job 狀態寫在 `data/jobs/tertiary/`；舊版 `data/jobs/dragen/` 紀錄仍可讀取。底層相容腳本仍名為 `run_stopgaps.sh`，但 log / UI 一律顯示為 post-processing。AnnotSV 的目前用法另見 `docs/annotsv_current_usage.md`。

三級分析重用既有 `/home/pipeline/tertiary_output/{sample}/` output 時，優先使用 UI sample ID（例如 `VAL-57-dragen` / `VAL-57-nckuh`）作為 pipeline output 目錄，且後端 job API / worker 會為新 job 強制補上來源 suffix，避免 DRAGEN 與 in-house 同名 sample 互相覆蓋；開跑前 reuse 只信任 exact UI sample ID 目錄，不再用無後綴 source-ID-only output 當 shortcut。reuse 也不再只看 `03_acmg/*.snv_indel.acmg.tsv`，而是檢查 SNV、Mito、CNV、SV；PGx checkbox 有勾時也會要求 PGx/PharmCAT 輸出存在，缺任一項就讓 Nextflow 用 `-resume` 補齊。若開跑前發現無後綴 source-ID-only 目錄存在、但目標 suffixed 目錄不存在，worker 會直接刪除該 legacy publish 目錄，讓 Nextflow 從自己的 work/cache 以 `-resume` 判斷並重新 publish。Nextflow 完成後若 pipeline 仍輸出到 source-ID-only 目錄，worker 會把它 rename 成 suffixed 目錄；已跑完但 pipeline output 還沒有 suffix 的 case，仍可 dry-run `python scripts/repair_pipeline_output_suffixes.py`，確認後加 `--apply` 搬移成 suffixed 目錄。工具會依 `pipeline_source.json` 修復，也會預設把 `/home/pipeline/tertiary_output/VAL-數字/` 視為 legacy DRAGEN 並搬移成 `VAL-數字-dragen/`（可用 `--legacy-dragen-pattern` 調整或設空字串關閉）。worker-owned log timestamp 會放在行尾 `[YYYY-MM-DD HH:MM:SS]`；Nextflow process 完成時只在原 stdout 行尾追加 elapsed 分鐘與時間戳，batch 進度條會依同一 process 的完成比例推進。
...
服務啟動時會以 daemon thread 在背景預熱 HPO、phenotype gene map、OMIM 與 mito ClinVar cache，避免解析大型 `phenotype_to_genes.txt` 期間擋住 HTTP port。完整 SNV TSV 的 gene search 走 per-sample `snv_gene_index.sqlite`；index 由 tertiary job 的 post-processing 預建，載入個案後不再自動掃描 WGS raw TSV。`snv_indel.review.tsv` 的 manifest 會記錄 test type 與 WES read-depth hard floor，SNV/Indel 會排除 chrM/MT，WES 樣本會在 review TSV 產生階段直接排除 DP < 20；SNV/Indel 主畫面、gene search 與 reviewer-marked 補點也都套用同一個 WES DP ≥ 20 gate。若舊樣本缺少或尚未更新 `snv_gene_index.sqlite`，報告區會對少量已標記 SNV id 做 raw TSV fallback scan，把 gene search 標記進 1/2/C 的點位補回 payload。Exomiser/LIRICAL 背景 job 會由完整 TSV 產生 `vcf_from_tsv.vcf.gz`，v3.5 多 transcript rows 會先依 `(CHROM,POS,REF,ALT)` 去重，避免同一 variant 在 VCF 重複出現；VCF sidecar 會記錄 writer version 與 TSV signature，舊版未去重 VCF 會自動重建。Exomiser/LIRICAL rerun 的 YAML templates 放在 repo 的 `phenotype_reference/`，render 失敗會把 traceback 寫入該 analysis version 的 `analysis_files/rerun.log`。Secondary findings 透過 `/api/samples/{id}/secondary-snv` 背景載入，來源是已產生的 `snv_indel.review.tsv`，再用 ACMG SF / 血脂 / 腫瘤 / 中風 / proactive / carrier panel gene、ClinVar P/LP 或卡片最終 ACMG P/LP、VAF ≥ 0.2、zygosity 非 ref 做篩選；因此不會為 secondary panels 掃完整 raw TSV，也不會把 BED 外、低 DP、低 VAF/ref 的位點帶進前端。PDF gene list 改由 `/api/samples/{id}/report-gene-list` lazy 載入，避免 `GET /samples/{id}` 首次載入同步解析 phenotype gene map。`in_panel` / `pheno_score` 由 active analysis 的 `pheno_score.tsv` 在載入與搜尋時補入，不再為 HPO/panel 變更重寫完整 raw TSV。刪除個案時只 invalidate 該 sample 的 SNV 與個案摘要 cache，不會同步重掃全部清單。
...
「載入既有資料」會先清空目前頁面的 HPO terms、fixed panel chips 與 free panel rows，再依新查詢結果填回；若下一位病人查無 phenotype，頁面會維持空白選擇，不會沿用上一位病人的 HPO/panel。
