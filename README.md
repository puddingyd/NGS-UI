...
| `NGS_UI_GPN_MSA_DB` | `$NGS_UI_HOME/biotools/gpn_msa/scores.tsv.bgz` | 必要的固定 GRCh38 GPN-MSA pre-computed score；同路徑 `.tbi` 與 `tabix` 也必須存在，只 join 最終 review TSV |
...
Nextflow 的 `--out_dir` 指向 job-private `.staging/{job_id}/`，不直接覆寫正式個案。結束後先驗證該 source sample 的 `00_prepare`–`06_cnv_sv` 目錄以及 81 欄 v3.6 SNV、Mito、STR、CNV、SV 產物，並核對 `DBNSFP_VERSION`；有勾 PGx 時再要求 `07_pgx` 與 PGx/PharmCAT 產物。`Research-only` 勾選時 Nextflow 加 `--academic_dbnsfp true`（5.3a），否則維持 4.9c；同一 checkbox 也控制 post-processing 是否補 SpliceAI。驗證通過後，SNV post-processing 在 staging 中完成 weekly ClinVar comparison、GeneBe、SpliceAI-only、GIAB、院內 AF、MANE、LitVar2、prefixed overlay/review/index、MITOMAP 與 ploidy 衍生檔；GPN-MSA 不受 Research-only 控制，所有 case 在 review TSV materialization 時都必須由固定 GRCh38 pre-computed BGZF/TBI 補分，缺資料或 tabix 會讓新三級 job 明確失敗。SpliceAI、GeneBe live API 與 LitVar2 僅處理共同 final-review predicate 留下的 variants；LitVar2 DB 缺失或查詢失敗仍為 warning/no-op。hidden working TSV 仍在 `finally` 刪除。最後進入 `promote-output`，以可 rollback 的 rename 切換正式 00–07 與 worker-owned 08 衍生檔，保留當下最新的 reviewer metadata、case summary 與 analyses，並最後才切換 `{LIS_ID}.layout.json`。任何 Nextflow、完整性驗證或 post-processing 失敗都只清理 staging，原本正式結果與已載入個案維持可用；批次切換中途失敗則還原同批已切換的 sample。若取消 PGx，舊 `07_pgx` 會在成功切換時移除，避免顯示上一次的 PGx 結果。三級分析清單會忽略 `.staging` / `.rollback`；完整刪除仍會清掉對應 pipeline/UI tree 與 job log，執行中的 sample 不可刪。

dbNSFP 完全由 Nextflow v3.6 負責：預設 4.9c；勾 `Research-only` 時用 5.3a 並提供 REVEL、MutPred2、VEST4、CADD_phred。Post-processing 的 `annotate_spliceai.py` 只讀 `$NGS_UI_HOME/biotools/spliceai/spliceai_scores.raw.{snv,indel}.hg38.vcf.gz` 及各自 `.tbi`，且只對 final-review variants 補 `SPLICEAI_MAX`，不載入 dbNSFP；未勾 Research-only 時整步略過，勾選但 reference/index 缺失則明確失敗。GPN-MSA 是另一條獨立、非 Research-only 的 final-review annotation：先執行 `scripts/download_gpn_msa_scores.sh` 將官方約 9 billion SNV pre-computed score BGZF/TBI 部署到固定路徑，之後每個 case 以 tabix join，不設自動更新 timer 或 release hierarchy。[官方 Hugging Face dataset card](https://huggingface.co/datasets/songlab/gpn-msa-hg38-scores) 標示 MIT license，可商用但仍應保留來源與引用；論文頁面的出版授權是另一件事。最新版 ClinVar 由 `ngs-ui-clinvar-update.timer` 每週三台北時間 04:00 下載官方 GRCh38 VCF、驗證並原子重建 `$NGS_UI_HOME/biotools/clinvar_latest/clinvar_latest.sqlite`；三級 post-processing 對整份 working TSV 串流比對，將最新分類、review status 與日期分開保存在 UI overlay，marker 寫入 `{LIS_ID}.clinvar_comparison.json`。最新版不覆寫 baseline ClinVar 或 tier，只驅動箭頭 hover；診斷／健檢 DOCX 的日期、ClinVar 值及健檢候選集合固定使用 Nextflow 20260720 基準。安裝 timer：`scripts/install_clinvar_update_timer.sh`。
