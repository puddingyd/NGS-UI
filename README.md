...
3. **看變異卡片** — 個案載入後先顯示 SNV/Indel（分段載入），CNV/SV、Mitochondria、STR 與 PGx 在背景載完後補上：
   ...
   - PGx / PharmCAT：讀取 pipeline `07_pgx/{sample}.pgx.tsv` 與精簡後的 `pharmcat.report.json`，依 gene 聚合 diplotype、phenotype、activity score 與 drug recommendations，分成 Clinically actionable、Routine / negative screens 與 Additional PharmCAT genes；完整 PharmCAT JSON 不放進核心樣本 payload。
