...
成大醫院基因醫學部的 NGS 三級分析判讀工具。次級 pipeline（Nextflow，跑在另一台 compute cluster）產出 per-sample 的註解 TSV，本平台讓 reviewer 載入個案、檢視 SNV/Indel + CNV/SV + Mitochondria + STR + PGx 變異、標記 causative / candidate / other、整理 ACMG SF / proactive / carrier secondary findings、撰寫判讀意見，並匯出診斷報告 (docx)。另附一個獨立的「臨床表徵輸入 (HPO / gene panel)」工具掛在 `/phenotype/`。
...
3. **看變異卡片** — 個案載入後先顯示 SNV/Indel（分段載入），CNV/SV、Mitochondria、STR 與 PGx 在背景載完後補上：
   ...
   - ACMG SF / Proactive / Carrier screening 從 `snv_indel.review.tsv` 產生，只列 custom panel `ACMG_SF_v3.3`、`proactive`、`carrier_mackenzie_1300+` 內的 SNV/Indel，且需符合 ClinVar P/LP 或卡片最終 ACMG P/LP（GeneBe 優先、再用 pipeline ACMG）、VAF ≥ 0.2、zygosity 非 ref。三個分析區預設展開並保留全部候選點位；ClinVar P/LP 預設 ✓，其他 ACMG P/LP 預設未勾，報告區只顯示 ✓ 點位。手動取消會寫入 `secondary_findings.{section}.dismissed`，不會下次載入又被預設勾回。
...
服務啟動時會以 daemon thread 在背景預熱 HPO、phenotype gene map、OMIM 與 mito ClinVar cache，避免解析大型 `phenotype_to_genes.txt` 期間擋住 HTTP port。完整 SNV TSV 的 gene search 走 per-sample `snv_gene_index.sqlite`；index 由 tertiary job 的 post-processing 預建，載入個案後不再自動掃描 WGS raw TSV。`snv_indel.review.tsv` 的 manifest 會記錄 test type 與 WES read-depth hard floor，WES 樣本會在 review TSV 產生階段直接排除 DP < 10；SNV/Indel 主畫面、gene search 與 reviewer-marked 補點也都套用同一個 WES DP ≥ 10 gate。Exomiser/LIRICAL 背景 job 會由完整 TSV 產生 `vcf_from_tsv.vcf.gz`，v3.5 多 transcript rows 會先依 `(CHROM,POS,REF,ALT)` 去重，避免同一 variant 在 VCF 重複出現；VCF sidecar 會記錄 writer version 與 TSV signature，舊版未去重 VCF 會自動重建。Exomiser/LIRICAL rerun 的 YAML templates 放在 repo 的 `phenotype_reference/`，render 失敗會把 traceback 寫入該 analysis version 的 `analysis_files/rerun.log`。Secondary findings 透過 `/api/samples/{id}/secondary-snv` 背景載入，來源是已產生的 `snv_indel.review.tsv`，再用 ACMG SF / proactive / carrier panel gene、ClinVar P/LP 或卡片最終 ACMG P/LP、VAF ≥ 0.2、zygosity 非 ref 做篩選；因此不會為 carrier panel 掃完整 raw TSV，也不會把 BED 外、低 DP、低 VAF/ref 的位點帶進前端。PDF gene list 改由 `/api/samples/{id}/report-gene-list` lazy 載入，避免 `GET /samples/{id}` 首次載入同步解析 phenotype gene map。`in_panel` / `pheno_score` 由 active analysis 的 `pheno_score.tsv` 在載入與搜尋時補入，不再為 HPO/panel 變更重寫完整 raw TSV。刪除個案時只 invalidate 該 sample 的 SNV 與個案摘要 cache，不會同步重掃全部清單。
