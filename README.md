...
3. **看變異卡片** — 個案載入後先顯示 SNV/Indel（分段載入），CNV/SV 與 Mitochondria 在背景載完後補上：
   ...
   - GeneBe ACMG 第二意見（`GENEBE_ACMG_*` 欄）改由本機 GeneBe 資料庫離線提供：`run_stopgaps.sh` 用 `tabix` 對 `NGS_UI_GENEBE_DB`（預設 `biotools/genebe/genebe_hg38.tsv.gz`，bgzip + tabix）點查 `acmg_score`/`acmg_criteria`，取代原本的 GeneBe API，不需帳號/網路。DB 為預算好的快取，查不到的點（如 novel coding indel）就沒有 GeneBe 第二意見，pipeline 自己的 ACMG 仍會顯示。部署需 `tabix`，且 `.tbi` 要比 `.gz` 新。
