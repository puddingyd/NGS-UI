...
3. **看變異卡片** — 個案載入後先顯示 SNV/Indel（分段載入），CNV/SV、Mitochondria、STR 與 PGx 在背景載完後補上：
   ...
   - ClinVar/ACMG 下方顯示 `LitVar2 (版本日期)`：標題維持純文字，旁邊的 `SquareArrowOutUpRight` 外部連結圖示可開啟該 variant 的 LitVar2 結果頁；前五個 PMID 分別直連 PubMed，超過五篇顯示 `and N others` 並連回 LitVar2。三級 post-processing 先只對 review filter 候選點查本地 bulk SQLite；有完成 marker 的個案在 gene search 時，會對尚無狀態的完整 TSV 搜尋結果做一次本地批次查詢。每張 SNV 卡片旁的 refresh icon 可強制以目前 DB 重查單點，舊個案也能使用；其他尚未查詢的舊點仍顯示 `NA (請重跑三級)`。按需查詢的 hit/no-match 依 DB fingerprint 存在 per-sample SQLite，DB 換版即自動失效；全程不呼叫 LitVar2 API，也不影響 tier、ACMG、排序或報告。
