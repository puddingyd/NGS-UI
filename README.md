...
2. **載入新個案** — 點「載入新個案」：
   ...
   - 旁邊的「個案清單」可查看已載入個案、依 `WES` / `WGS` 篩選並全文搜尋；表格會摘要目前 active analysis 的 HPO/panel（HPO 顯示如 `Seizure HP:0001250`）、causative / other SNV、CNV、SV、Mito，已勾選 OMIM disease、Mito ClinVar disease 與 CNV/SV reviewer 輸入的 Disease、主畫面 comment、簽收與載入時間。多個 HPO/panel、variant / disease 會逐行顯示，長 HGVS 可自動折行。表格內「刪除」會移除 `sample_metadata.json`、`case_summary.json` 與 `analyses/` 等註冊/報告狀態，但保留 `NGS_UI_HOME/tertiary_output/{LIS_ID}/` 內的 annotation、PGx、ploidy 等三級輸出檔；完成後該 sample 會從個案清單消失並回到「載入新個案」的未登錄清單。
...
3. **看變異卡片** — 個案載入後先顯示 SNV/Indel（分段載入），CNV/SV、Mitochondria、STR 與 PGx 在背景載完後補上：
   ...
   - 「匯出健檢報告」會先跳出選擇畫面，預設勾 ACMG SF 與藥物基因體學，血脂/腫瘤/中風/Carrier/Proactive 預設不勾，彈窗選單顯示「ACMG 疾病風險基因」。DOCX 保留原章節骨架、固定寬度 `====` ASCII 表格、表格下方列點、檢測方法、注意事項與基因清單；頁首保留院名及「基因醫學部基因檢測分析研究報告」副標題。檢驗套組會依 UI 勾選自動組合「ACMG疾病風險基因」與「藥物基因體學」。ACMG SF 共用警語置於第一、第二類之前，內容使用次發現基因清單第 3.3 版（2025 年發表）、ClinVar 版本及 P/LP 回報範圍；變異表格的 ACMG/AMP 分級維持英文，表後說明與附錄參考資料使用中文。同基因多位點合併在同一表格並共用一組綜合說明，EXON 為 `.` 等缺值時會改讀 INTRON。ACMG 主文分成「第一類：與疾病風險相關之致病性或疑似致病性變異位點」與「第二類：符合帶因者狀態之致病性或疑似致病性變異位點」：AD、X-linked、純 AR 同基因兩個以上變異及純 AR homozygous 變異列入第一類，純 AR 單一 heterozygous 變異列入第二類；第一類標題與結果之間不留空行，第一類結果與第二類標題之間保留一行。原六組分類與完整基因改列於 §五基因清單。完整檢測限制只在 ACMG SF 全段末尾輸出一次，所有變異位點參考資料依第一類、第二類主文順序集中於報告末端附錄。健檢 PGx 內容完全取自 `pharmcat.report.json`：主文依序為「用藥建議概覽」、「藥物建議摘要」及「基因型與表現型」；摘要以固定寬度 `====` ASCII 表格列出「藥物 / 基因 / 建議處置 / 建議依據及等級」，一個藥物一列並依藥物英文名稱 A-Z 排序，且只列 CPIC Strong/Moderate 與 FDA therapeutic-management，FDA-label-only 藥物只留在附錄完整建議。摘要與完整建議只列實際驅動處置的基因，正常或低風險表型不會因同一藥物的另一基因需調整而被誤列。固定 21 個 CPIC Level A genes 的 phenotype 欄優先使用 JSON phenotype，`n/a` 或空值時改用 allele function，兩者皆無時顯示 `No phenotype assigned`；MT-RNR1 在 JSON 無有效結果時可由 TSV 補值。「用藥建議概覽」保留「其餘未列之藥物，未發現符合本報告回報規則之明確處方調整建議。」CPIC、FDA 官方查詢網址與用藥警語置於 PGx 主文末端並顯示完整網址，CPIC 排列第一；報告末端附錄依序為「變異位點參考資料」與 drug-centric「完整用藥建議」，兩者之間保留一行且不使用附錄一、二編號。ASCII 表格不設定 keep-with-next，避免 Word 顯示段落左側黑色方塊。報告基因範圍固定為目前 21 個至少具有一組 CPIC Level A gene–drug pair 的基因，不含 CYP3A4 與 IFNL3。
