...
3. **看變異卡片** — 個案載入後先顯示 SNV/Indel（分段載入），CNV/SV、Mitochondria、STR 與 PGx 在背景載完後補上：
   ...
   - PGx / PharmCAT：以精簡後的 `pharmcat.report.json` 為主，後端直接共用健檢 DOCX 規則產生固定 21 個 CPIC Level A genes 的畫面資料。報告區顯示用藥建議概覽、藥物摘要及基因型/表型表；分析區顯示完整 CPIC/FDA 建議與 21 genes 的 allele、variant、uncalled/messages。pipeline `07_pgx/{sample}.pgx.tsv` 只在 MT-RNR1 的 JSON 為 Unknown/No Result 時補基因型與風險，不再決定其他 gene 的主畫面範圍。
