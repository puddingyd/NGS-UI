...
3. **看變異卡片** — 個案載入後先顯示 SNV/Indel（分段載入），CNV/SV、Mitochondria、STR 與 PGx 在背景載完後補上：
   ...
   - 「匯出健檢報告」會先跳出選擇畫面，預設勾 ACMG SF 與藥物基因體學，血脂/腫瘤/中風/Carrier/Proactive 預設不勾。DOCX 保留原診斷報告名稱及章節骨架、固定寬度 `====` ASCII 表格、表格下方列點、檢測方法、注意事項與基因清單；檢驗套組會依 UI 勾選自動組合「可採取醫療處置之疾病風險基因」與「藥物基因體學」。ACMG SF 固定警語說明 v3.3、ClinVar 版本及 P/LP 回報範圍；同基因多位點合併在同一表格並共用一組綜合說明，結構欄依 transcript 顯示 exon 或 intron，完整檢測限制只在 ACMG SF 全段末尾輸出一次。健檢 PGx 內容完全取自 `pharmcat.report.json`：narrative 收 CPIC Strong/Moderate 且具有 dosing/alternate-drug/other-guidance action 的非標準處置，以及 FDA Section 1 therapeutic-management；完整表格按基因保留所有匹配的 CPIC、FDA Label 與 FDA PGx Association 原文。報告基因範圍固定為目前 22 個至少具有一組 CPIC Level A gene–drug pair 的基因，不含 CYP3A4。
