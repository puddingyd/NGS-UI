...
Output txt lands in PHENOTYPE_DIR as {mrn}_phenotype.txt whenever an MRN is
available. LIS-only naming remains a no-MRN fallback, and old LIS/MRN files
remain readable without being rewritten or deleted.
"""
...
def _check_token(name: str, value: str, *, required: bool) -> str:
    try:
        return patient_phenotype_store.check_token(
...
def save_phenotype_file(payload: dict):
    ...
    Body: {mrn, code (LIS_ID), content}. MRN is the canonical patient key;
    LIS_ID is retained only as a fallback when MRN is unavailable.
    """
    ...
    try:
        return patient_phenotype_store.save_text(
            mrn=mrn,
            code=code,
...
def load_phenotype_file(
    ...
    Lookup order: exact {mrn}_phenotype.txt → legacy LIS/MRN forms.
    ...
    try:
        data = patient_phenotype_store.load(mrn=mrn, code=code)
    except ValueError as exc:
        raise HTTPException(400, str(exc)) from exc
    if not data:
