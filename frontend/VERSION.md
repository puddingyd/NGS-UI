...
- 健檢報告調整 ACMG SF 參考資料位置；PGx 基因型表支援 MT-RNR1 由 TSV 補值，phenotype 空值會改用 allele function 或顯示 `No phenotype assigned`；FDA 建議改為 therapeutic management 先於 label，並移除原文外層引號。
