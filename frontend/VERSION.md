...
- CNV／SV gene 表的 OMIM 疾病改為透明底、細分隔的表格內樣式；每個基因預設顯示前 2 個疾病，可按「展開其餘」查看並再次收合。
- 「已知致病區域重疊」每組預設顯示前 5 個疾病，其餘改用相同的計數式展開／收合；疾病勾選與 DOCX 輸出規則不變。

### v9.29 — 2026-09-26

- CNV／SV gene 表的 Phenotype 改以院內 `OMIM.xlsx` 為優先來源，保留 AnnotSV 原值作缺檔／查無資料時的 fallback；每筆 OMIM 疾病可直接勾選發報告。
- 「已知致病區域重疊」的 P_loss／P_gain／P_ins 疾病也可勾選發報告。DOCX 在原本 Disease 文字位置輸出「OMIM 勾選＋致病區域勾選＋自由文字」的去重聯集，以「、」連接，不增加報告段落；個案清單摘要同步採用相同聯集。

### v9.28 — 2026-09-26

- Clinical presentation 的 Documents 圖片預覽新增 50%–400% 放大縮小、比例顯示、重設與四方向移動按鍵；放大後可用滑鼠或手指直接拖曳，亦支援雙擊、`+`／`-`／`0`、Shift＋方向鍵及 Ctrl／⌘＋滾輪。切換檔案或 TIFF 頁面時自動回到符合視窗的 100%，原有左右換檔與 TIFF 翻頁維持不變。
...
- 新卡片只顯示整合後座標、Filter／Qual 與支持證據；Rule B 保留原本 Filter。既有事件 ID 保留，原始片段可展開查看；舊個案需重跑 CNV post-processing／三級分析後才套用新註解。

### v9.24 — 2026-09-24

- CNV／SV 相鄰片段整合新增小幅邊界交疊規則：交疊同時不超過 10 kb 與較短片段的 10% 時也會合併；原有 250 kb gap 規則保留。
- 這項比例只避免短 CNV 的主要區段互相重疊時被誤併，不參與致病性判斷；高度交疊、包含關係與不同型別／來源仍維持分開。

### v9.23 — 2026-09-24

- 診斷報告會把同一類別內的同基因 SNV/Indel 合併成一個標題與一張多列表格；重複疾病說明只列一次，不同 ACMG 分級會一起顯示，例如「致病性及疑似致病性」。
- 第一類沒有結果時改為「未找到與臨床症狀相關基因之已知致病性變異位點。」；表格中的全形破折號改按兩格寬度對齊，不再讓下一欄前多出一格。

### v9.22 — 2026-09-23

- CNV／SV 影響篩選改為單行英文勾選列，顯示各類別數／全部事件數並採半透明底色；移除標題、額外按鈕及說明文字。
- 卡片影響描述改為英文類別與基因括號說明，例如「Exonic / splicing · 臨床相關基因：SLC25A24 (coding exon) · HPO match 42.9 / 100」。

### v9.21 — 2026-09-23

- CNV／SV 的 Clinical、Pathogenic 四個分頁新增影響類型勾選，適用所有 DRAGEN／NCKUH、WGS／WES；預設顯示外顯子／剪接影響與位置註解不足，UTR／內含子可勾選展開。
- 新增顯示全部、恢復預設、顯示／全部事件數與卡片影響說明；Clinical 的 P／LP 及所有分頁已標記 1／2／C 的事件保持可見；Pathogenic 的其餘事件依勾選篩選。缺註解不當成無影響，合併卡片沿用原始片段證據。
- 分析畫面改用固定優先順序，HPO 符合度與 panel 分開，不再依樣本最高 AnnotSV 分數縮放排序；既有 Clinical／Pathogenic 分類、ACMG 和報告選取不變。重新載入個案即可套用。

### v9.20 — 2026-09-22

- 三級分析清單的「查看 Log」改為顯示該次完整 NGS-UI job log，不再只取最後數百行；開啟時從第一行開始顯示。
- 「查看 Log」旁新增「下載 .nextflow.log」；每次 Nextflow 成功、失敗或終止時，都會在釋放共享 cache lock 前保存該 job 專屬快照，避免後續批次輪替成別次執行的 log。改版前未保存快照的舊 job 不會下載共享目錄裡可能屬於其他 run 的檔案。

### v9.19 — 2026-09-22

- DRAGEN 三級 post-processing 新增 CNV rescue：保留僅未達長度／品質門檻、且有完整 SV 配對或符合雙向重疊 ≥50% 等支持規則的 CNV；in-house 維持原流程。
- 新增事件補做 AnnotSV，卡片可展開查看原始 Filter／Qual、整合前後座標與支持 SV；合併卡片保留各片段證據。不新增待檢視候選區、不自動提高 ACMG 或勾入報告。
- 重跑會更新 rescue 結果；既有判讀與原始三級輸出保留。舊個案重新執行三級分析後生效。

### v9.18 — 2026-09-21

- 載入新個案清單改用伺服器共用索引，先顯示清單並背景更新；選取檢體後才載入 HPO／panel 與個案詳細資料，縮短等待時間。
- 登錄、取消登錄與三級分析完成／刪除會更新清單；編號對照更新不再重掃 pipeline，仍可按「更新清單」強制重新掃描。

### v9.17 — 2026-09-21

- 載入新個案時按「EMR 同步」，會一起依病歷號帶入臨床表徵頁面已儲存的 HPO／gene panel，並顯示 Clinical presentation 預覽；已儲存表徵優先於 EMR。
- EMR 連線失敗時仍可載入已儲存表徵；切換病歷號或檢體後不套用舊同步回應。

### v9.16 — 2026-09-18

- 臨床表徵頁面同時填病歷號與檢體編號後按「儲存」，會建立編號連結；載入新個案時選取檢體即可自動帶入病歷號與既有 HPO/panel。
- 載入新個案會自動檢查連結更新；支援完整檢體編號，保留個案清單原有資料，並提示既有病歷號對應衝突。
...
- ClinVar 統一縮寫為 P／LP／VUS／LB／B，保留各判讀筆數與星等，例如 `P(22)|VUS(1)(1★)`。各段文字分別套用紅系／黃／綠系底色，混合判讀也能直接辨認；滑鼠停留可查看完整名稱。套用於共用 SNV/Indel 與 Mito 卡片，不改變分類、tier 或 DOCX 報告資料。

### v9.12 — 2026-09-14

- ACMG SF 附錄的「參考資料」標題改為與疾病內文相同的 11 pt 粗體，前方增加一個空白行（共兩行）；文獻清單縮為 9 pt，網址為 8 pt，與疾病介紹作出區隔。

### v9.11 — 2026-09-14

- Secondary findings 在 ACMG SF 與中風相關基因之間新增「遺傳癌症 v2.0」，使用固定 209-gene panel；分析區、報告區與左側選單同步加入，Carrier 名稱改為 Carrier screening。
- 遺傳癌症與 ACMG SF、中風、Carrier screening 共用候選篩選及預設勾選規則；同一點位跨 panel 的勾選與判讀同步，匯出時合併去重。
- 健檢報告新增「遺傳癌症 v2.0」選項，預設不勾選；勾選才輸出其點位與完整基因清單。ACMG SF 衛教及 PGx 仍各自依勾選輸出。
- 開發與推送改為直接使用遠端 default branch，先前報告修改已合入。

### v9.10 — 2026-09-14

- 健檢報告依勾選輸出 ACMG SF、PGx 的主文、基因清單與附錄；未勾 PGx 時，檢測方法中的藥物基因相位及 CYP2D6 說明也一併省略，編號保持連續。
- ACMG 附錄沿用最新核定版本；匯出選單補上 ACMG SF／PGx 名稱及主文、附錄隨勾選輸出的提示。空白或無效選項不再自動加入預設項目。

### v9.9 — 2026-09-14

- 依第二輪 DOCX 追蹤修訂與 4 則註解更新 ACMG 附錄：同步心肌病變名稱、簡化疾病表現與追蹤治療說明，結節性硬化症與視網膜退化的指定詞句改為民眾容易理解的文字。
- 疾病間固定空一行；依修訂刪除副神經節瘤補充說明，文末改為「參考資料」並移除查閱日期說明。保留六類索引、36 個疾病群組、84 個基因及原文獻連結。
- 同日小修：NF2 中文名稱更新為「神經纖維瘤症候群第二型」；參考資料改為接續最後一個疾病、空一行，不再強制另頁。

### v9.8 — 2026-09-11

- 依修訂文件更新健檢 ACMG 附錄：逐段套用追蹤修訂與 6 則註解，合併三種瘜肉症為一組，索引改為 36 個疾病群組並保留全部 84 個基因，統一使用「瘜肉」。
- 標題改為「ACMG疾病風險基因與相關疾病簡介」，索引說明移至索引標題下方，中英文疾病標題改為同段、同字級。依修訂刪除遺傳模式通則、逐疾病來源編號與指定補充文字，保留文末參考資料及索引跳轉；疾病介紹仍放在變異位點參考資料與完整用藥建議之間。

### v9.7 — 2026-09-08

- 健檢 DOCX 附錄新增 ACMG SF v3.3 疾病與健康照護簡介：六類索引、38 個疾病群組，涵蓋 84 個基因。以民眾可理解的文字說明疾病、遺傳模式、可能表現、病程、追蹤與治療，並附基因差異及資料來源。
- 順序為變異位點參考資料 → ACMG 疾病簡介及文獻 → 完整用藥建議。勾選 ACMG 時完整附上，無回報位點也會加入；不加逐基因檢出狀態。電子索引可跳轉疾病短文，文獻編號可跳至來源，疾病短文保持同頁便於閱讀。
