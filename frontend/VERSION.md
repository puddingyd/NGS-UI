...
- IGV BAM tracks 新增「其他路徑」下拉，可選 DRAGEN run 或 in-house batch/sample BAM 資料夾；選 primary 後同 batch 加入清單也改用同資料夾。
- IGV 對 `-dragen` 後綴 sample 會強制使用 DRAGEN BAM roots，避免舊樣本 fallback 到同 base ID 的 in-house BAM；Mitochondria variant card 也支援 IGV。
