...
- 健檢 PGx 主文改為「用藥建議概覽、藥物建議摘要、基因型與表現型」順序；完整用藥建議與所有變異位點參考資料集中到報告末端附錄，並新增 CPIC/ClinPGx、FDA PGx 關聯表、FDA 生物標記表及 Drugs@FDA 官方查詢連結與用藥警語。
- 健檢報告的 ACMG 五級文字統一顯示為良性、疑似良性、不確定意義、疑似致病性、致病性；Secondary findings 維持 ClinVar P/LP 預設勾選、僅 ACMG P/LP 的候選點位預設不勾選。
